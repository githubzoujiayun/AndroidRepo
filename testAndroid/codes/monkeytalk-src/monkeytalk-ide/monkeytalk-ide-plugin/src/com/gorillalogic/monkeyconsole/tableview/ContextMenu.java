...
public class ContextMenu {
	...
	public void show()
	{
		this.controller = FoneMonkeyPlugin.getDefault().getController();
		...
		if (item != null){
			...
			if(num > 0){
				...
				playMenuItem.setEnabled(controller.isCurrentlyConnected() 
						&& !controller.isRecordingON() && !controller.isReplayON() 
						&& dmconsole.getLimitedComponentSet() == null);
				playMenuItem.addSelectionListener(new SelectionAdapter() { 
						public void widgetSelected(SelectionEvent event) {
							Table table = dmconsole.getTv().getTable();
							controller.startReplayRange(
									table.getSelectionIndex(), 
									table.getSelectionIndex() + table.getSelectionCount());
						}
					});
			}
		}
