...
public class CloudServices {
	private static final int DEFAULT_TIMEOUT = 15000; // default timeout in
													  // ms for timed calls

	static String learnMoreUrl = "http://www.cloudmonkeymobile.com/cloud/overview";
	public static String registerUrl = "https://www.cloudmonkeymobile.com/cloud-register";
	static String acceptTermsUrl = "https://www.cloudmonkeymobile.com/cloudmonkey/license-agreement";
	static String logEventsUrl = "NO_EVENT_LOGGING" + CloudConstants.LOG_EVENT;
	...
	protected static String getControllerUrlRoot() {
		...
		return protocol + "://" + CloudServices.getControllerHost() + ":"
	...
	public static JSONObject getDeviceTypes() throws CloudServiceException {
		String errorMessage = getCloudUnavailableErrorMessage();
		try {
	...
	/**
	 * Ping is an alias for getDeviceTypes(). This simply hits a valid CMA
	 * endpoint to establish availability. Times out after
	 * {@link #DEFAULT_TIMEOUT} ms.
	 * 
	 * @throws CloudServiceException
	 *             if the ping timed out or failed for another reason
	 */
	public static void ping() throws CloudServiceException {
		
		Callable<Object> pinger = new Callable<Object>() {
			@Override
			public Object call() throws CloudServiceException {
				getDeviceTypes();
				return null;
			}
		};
		...
		timedCall(pinger, DEFAULT_TIMEOUT, errorMessage);
	}

	private static String getCloudUnavailableErrorMessage()
	{
		return "CloudMonkey LabManager at " + CloudServices.getControllerHost()
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					// log("exception caught in logEventAsync(), ignoring. Stack trace for diagnostic purposes:",
					// e);
				}
			}
		}).start();
	 ...
	 */
	private static JSONObject logEvent(String tag, String data, Date timestamp)
			throws CloudServiceException {
	...
	private static JSONObject logEventInternal(String tag, String data, Date timestamp)
		...
		try {
			...
			// removed 09/2014 Rally Ticket US691
	...
	/**
	 * Attempts to log in using a timeout of {@link #DEFAULT_TIMEOUT} ms.
	 * 
	 * @return the login token
	 * @throws CloudServiceException
	 *             if the login attempt timed out or failed for another reason
	 */
	public static String login(final String username, final String password)
			throws CloudServiceException {

		Callable<String> loggerInner = new Callable<String>() {
			@Override
			public String call() throws CloudServiceException {
		...
		return timedCall(loggerInner, DEFAULT_TIMEOUT, errorMessage);
	}
	
	private static String loginImpl(String username, String password)
			throws CloudServiceException {
	...
	/**
	 * Executes the callable on a new thread with the specified timeout.
	 * {@link CloudServiceException}s thrown by {@link Callable#call()} are
	 * re-thrown by this method. All other exceptions are wrapped in a
	 * CloudServiceException and re-thrown.
	 * 
	 * @param errorMessage
	 *            the error message to use when wrapping thrown Exceptions in
	 *            CloudServiceExceptions
	 * @throws CloudServiceException
	 *             if the execution timed out or threw an exception
	 */
	private static <T>T timedCall(Callable<T> callable, int timeoutMillis, String errorMessage) throws CloudServiceException
	{
		T answer;
		...
		try
		{
			...
			Future<T> futureResult = executor.submit(callable);
			answer = futureResult.get(timeoutMillis, TimeUnit.MILLISECONDS);
		} catch (InterruptedException e) {
			throw new CloudServiceException(errorMessage, e);
		} catch (TimeoutException e) {
			String timeoutMessage = errorMessage + " - bad hostname or port";
			throw new CloudServiceException(timeoutMessage, e);
		} catch (ExecutionException e) {
			Throwable cause = e.getCause();
			if (cause instanceof CloudServiceException)
			{
				throw (CloudServiceException) cause;
			}
			else
			{
				throw new CloudServiceException(errorMessage, cause);
			}
		}
		finally
		{
