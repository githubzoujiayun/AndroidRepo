...
public class MetaAPIGenerator {
	...
	private static void appendTypes(JavaClass clazz, StringBuilder sb) {
		StringBuilder types = new StringBuilder();
		for (DocletTag tag : clazz.getTagsByName("types")) {
			for (String type : tag.getValue().split(",")) {
				types.append(',').append("ScriptType.").append(type.trim().toUpperCase());
			}
		}
		if (types.length() > 0) {
			types.setCharAt(0, '(');
			sb.append(",\n  EnumSet.of").append(types).append(')');
		} else {
