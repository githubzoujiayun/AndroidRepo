...
public class RunAntGlobalsIT extends BuildFileTest {
	...
	public void testGlobals() {
		...
		assertThat(getOutput(), not(containsString("loading globals.properties")));
		assertThat(getOutput(), containsString("loading globals: foo, bar"));
	...
	public void testGlobalsWithBadArg() {
		...
		assertThat(getOutput(), containsString("www.gorillalogic.com"));
		assertThat(getOutput(), not(containsString("loading globals.properties")));
		assertThat(getOutput(), containsString("loading globals: foo"));
		assertThat(getLog(), containsString("-run: glob.mt"));
	...
	public void testGlobalsWithPropertiesFile() throws IOException {
		File props = new File("resources/test/globals.properties");
		FileUtils.writeFile(props, "foo=123\nbar=Bo Bo\n");

		// run the testGlobalsWithPropertiesFile target in resources/test/build.xml
		executeTarget("testGlobalsWithPropertiesFile");
		assertThat(getLog(), containsString("server=" + HOST + ":" + PORT));
		assertThat(getOutput(), containsString("www.gorillalogic.com"));
		assertThat(getOutput(), containsString("loading globals.properties"));
		assertThat(getOutput(), not(containsString("loading globals:")));
		assertThat(getLog(), containsString("-run: glob.mt"));
		assertThat(getLog(), containsString("Button 123 Tap \"Bo Bo\""));
		assertThat(getLog(), containsString("-end: glob.mt"));
		assertThat(getOutput(), containsString("result: OK"));
	...
	public void testGlobalsWithPropertiesFileOverriddenByAttribute() throws IOException {
		File props = new File("resources/test/globals.properties");
		FileUtils.writeFile(props, "foo=123\nbar=Bo Bo\n");

		// run the testGlobalsWithPropertiesFileOverriddenByAttribute target
		executeTarget("testGlobalsWithPropertiesFileOverriddenByAttribute");
		assertThat(getLog(), containsString("server=" + HOST + ":" + PORT));
		assertThat(getOutput(), containsString("www.gorillalogic.com"));
		assertThat(getOutput(), containsString("loading globals.properties"));
		assertThat(getOutput(), containsString("loading globals: foo"));
		assertThat(getLog(), containsString("-run: glob.mt"));
		assertThat(getLog(), containsString("Button 456 Tap \"Bo Bo\""));
		assertThat(getLog(), containsString("-end: glob.mt"));
		assertThat(getOutput(), containsString("result: OK"));

		assertThat(server.getCommands(), notNullValue());
		assertThat(server.getCommands().size(), is(1));
		assertThat(server.getCommands().get(0).getCommand(), is("Button 456 Tap \"Bo Bo\""));
