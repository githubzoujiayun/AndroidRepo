...
- (void)mtPickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    BOOL shouldRecord = (![self isKindOfClass:objc_getClass("_UIDatePickerView")] && ![self isKindOfClass:objc_getClass("UIDatePickerView")]
                         && ![self isKindOfClass:objc_getClass("UIWebSelectSinglePicker")]);
    ...
    if (![self isKindOfClass:objc_getClass("UIDatePickerView")] &&
        ...
        if ([event.command isEqualToString:MTCommandSelectIndex ignoreCase:YES]) {
            ...
            if ([event.args count] > 1) {
                // if we are given a second arg, use it as component value
                component = [[event.args objectAtIndex:1] intValue] - 1;

                for (int j = 0; j < [self numberOfRowsInComponent:component]; j++) {
                    NSString *currentTitle = @"";

                    if ([self.delegate respondsToSelector:@selector(pickerView:titleForRow:forComponent:)]) {
                        currentTitle = [self.delegate pickerView:self titleForRow:j forComponent:component];
                    }
        ...
        if ([self.delegate respondsToSelector:@selector(origPickerView:didSelectRow:inComponent:)]) {
