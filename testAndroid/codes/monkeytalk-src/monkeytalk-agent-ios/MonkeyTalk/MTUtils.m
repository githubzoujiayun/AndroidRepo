...
+ (UIImage *)screenDump {
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)])
        UIGraphicsBeginImageContextWithOptions([self rootWindow].bounds.size, NO, [UIScreen mainScreen].scale);
    else
        UIGraphicsBeginImageContext([self rootWindow].bounds.size);
    [[self rootWindow].layer renderInContext:UIGraphicsGetCurrentContext()];
...
+ (UIImage *)screenshot {
    CGImageRef (*UIGetScreenImage)() = (CGImageRef (*)())dlsym(RTLD_DEFAULT, "UIGetScreenImage");
...
+ (BOOL)isOs7Up {
    NSString *osVersion = [[UIDevice currentDevice] systemVersion];
    if ([[osVersion substringToIndex:1] intValue] >= 7) {
        return YES;
    }
...
+ (BOOL)isMinimumVersion:(NSString *)minimumVersion {
    NSArray *minVersions = [minimumVersion componentsSeparatedByString:@"."];
    NSString *osVersion = [[UIDevice currentDevice] systemVersion];
    NSArray *versions = [osVersion componentsSeparatedByString:@"."];
    
    for (int i = 0; i < minVersions.count; i++) {
        NSInteger minV = [[minVersions objectAtIndex:i] integerValue];
        NSInteger osV = versions.count > i ? [[versions objectAtIndex:i] integerValue] : 0;
