...
- (NSString *)xpathBase {
    NSString *xpathBase = @"(";
    NSInteger i = 0;
    for (NSString *xp in self.xPathBasePaths) {
        xpathBase = [xpathBase stringByAppendingString:xp];
        i++;
        
        if (i < self.xPathBasePaths.count) {
            xpathBase = [xpathBase stringByAppendingString:@"|"];
        }
    }
    xpathBase = [xpathBase stringByAppendingString:@")"];
    return xpathBase;
...
- (NSArray *)xPathBasePaths {
    return @[self.mtEvent.component.lowercaseString];
}

+ (NSDictionary *)webAutomators {
    NSArray *objects = [NSArray
        arrayWithObjects:@"MTButtonAutomator", @"MTCheckBoxAutomator", @"MTCheckBoxAutomator",
                         @"MTImageAutomator", @"MTInputAutomator", @"MTItemSelectorAutomator",
                         @"MTLabelAutomator", @"MTLinkAutomator", @"MTButtonSelectorAutomator",
                         @"MTTableAutomator", @"MTTextAreaAutomator", @"MTButtonSelectorAutomator",
                         nil];

    NSArray *keys = [NSArray
        arrayWithObjects:MTComponentButton, MTComponentToggle, MTComponentCheckBox,
                         MTComponentImage, MTComponentInput, MTComponentSelector, MTComponentLabel,
                         MTComponentLink, MTComponentButtonSelector, MTComponentTable,
                         MTComponentTextArea, MTComponentRadioButtons, nil];
...
- (NSString *)htmlTag {
    NSString *mtComponent =
        [MTConvertType convertedComponentFromString:mtEvent.className isRecording:YES];
...
- (NSString *)basePath {
    ...
    NSString *path = [NSString stringWithFormat:@"[%@]", [self finderExpression:mid]];

    NSInteger find = 0;
    if ([self isOrdinal]) {
        find = [[mtEvent.monkeyID substringFromIndex:1] integerValue];
        // find -= currentOrdinal;
        path = [NSString stringWithFormat:@"[%i]", (int)find];
    }
...
- (NSString *)finderExpression:(NSString *)selection {
    return [NSString stringWithFormat:@"@id='%@' or @name='%@' or @value='%@' or text()='%@' or "
                                      @"@title='%@' or @class='%@' or @alt='%@' or @src='%@' or "
                                      @"@href='%@'",
...
- (NSString *)xPath {
    ...
    convertedCommand = [NSString stringWithFormat:@"//%@%@", self.htmlTag, path];

    if ([[mtEvent.monkeyID lowercaseString] rangeOfString:@"xpath="].location != NSNotFound) {
        // Use Monkey ID as xpath
        convertedCommand =
            [mtEvent.monkeyID stringByReplacingOccurrencesOfString:@"xpath=" withString:@""];
    }
    ...
    if ([mtEvent.command isEqualToString:MTCommandEnterText ignoreCase:YES])
        ...
        seleniumCommand = @"setChecked:";
             ...
             ([mtEvent.command isEqualToString:MTCommandSelect ignoreCase:YES] &&
              !([mtComponent isEqualToString:MTComponentTable ignoreCase:YES] ||
                [mtComponent isEqualToString:MTComponentButtonSelector ignoreCase:YES])))
...
- (NSDictionary *)args {
    if (mtEvent.args != nil && [mtEvent.args count] > 0) {
        NSDictionary *argsDict =
            [NSDictionary dictionaryWithObject:[mtEvent.args objectAtIndex:0] forKey:@"value"];
...
- (MTElement *)elementFromXpath:(NSString *)xp {
    ...
    if (mtEvent.monkeyOrdinal) {
        NSArray *elements =
            [virtualDirectory findElements:webElementDict root:nil implicitlyWait:0];
    ...
    if ([response objectForKey:@"ELEMENT"]) {
        return [MTElement elementWithId:[response objectForKey:@"ELEMENT"]
                             andSession:session
                 andMTWebViewController:self.delegate
                           andCommandId:using];
    }
...
- (NSArray *)allElements {
    @autoreleasepool {
        ...
        NSString *using = @"xpath";
        NSString *value = [self.xPath stringByReplacingOccurrencesOfString:self.basePath withString:@""];
        ...
        for (NSDictionary *response in results) {
            [elements addObject:[MTElement elementWithId:[response objectForKey:@"ELEMENT"]
                                              andSession:session
                                  andMTWebViewController:self.delegate
                                            andCommandId:using]];
        }
...
- (void)scrollToElement:(MTElement *)e touchPoint:(CGPoint)touchCenter {
    ...
    BOOL isOffScreen = [webController isElementOffScreen:e] ||
                       !CGRectContainsPoint(webController.webView.bounds, touchCenter);
    if (isOffScreen) {
        ...
        if (scrollView.contentSize.width - origin.x < scrollView.frame.size.width) {
            origin.x = origin.x -
                       (scrollView.frame.size.width - (scrollView.contentSize.width - origin.x));
        }
        if (scrollView.contentSize.height - origin.y < scrollView.frame.size.height) {
            origin.y = origin.y -
                       (scrollView.frame.size.height - (scrollView.contentSize.height - origin.y));
        }

        // we can alternatively use scrollRectToVisible: but causes playback issues if element is
        // clipped
        //[scrollView scrollRectToVisible:CGRectMake(origin.x, origin.y, size.width, size.height)
        // animated:YES];
...
- (BOOL)playBackOnElement {
    ...
    if ([mtEvent.command isEqualToString:@"find" ignoreCase:YES]) {
        ...
        [self tapElement];
    } else if ([mtEvent.command isEqualToString:@"scroll" ignoreCase:YES]) {
        if (self.mtEvent.args.count < 2) {
            self.mtEvent.lastResult = @"Scroll action requires 2 args";
            return YES;
        }
        [self.element scrollElement:self.mtEvent.args];
    } else if ([mtEvent.command isEqualToString:MTCommandGet ignoreCase:YES] ||
               [mtEvent.command.lowercaseString rangeOfString:[MTCommandVerify lowercaseString]]
                       .location != NSNotFound) {
...
- (void)valueForElement {
    mtEvent.value = [self valueForElement:self.element];
}
...
- (NSArray *)argIndices:(NSString **)attribute {
    ...
    NSRegularExpression *regex =
        [[NSRegularExpression alloc] initWithPattern:@"\\((\\d+)[,\\)]"
                                             options:NSRegularExpressionCaseInsensitive
                                               error:nil];
    NSTextCheckingResult *r =
        [regex firstMatchInString:*attribute options:0 range:NSMakeRange(0, (*attribute).length)];
    NSString *argItems = nil;

    if (r) {
        argItems =
            [*attribute substringWithRange:NSMakeRange(r.range.location,
                                                       (*attribute).length - r.range.location)];
        ...
        NSRegularExpression *regex =
            [[NSRegularExpression alloc] initWithPattern:@"\\d"
                                                 options:NSRegularExpressionCaseInsensitive
                                                   error:nil];
        [regex
            enumerateMatchesInString:argItems
                             options:0
                               range:NSMakeRange(0, argItems.length)
