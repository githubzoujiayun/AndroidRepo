...
- (id)initInView:(UIView *)view at:(CGPoint)point withCount:(NSInteger)count {
	...
	if (self != nil)
	{
        ...
        if ([view isKindOfClass:[UITableViewCell class]]) {
            ...
            target = [view.window hitTest:point withEvent:nil];
        }
...
+ (void)performTouch:(UITouch*)touch {
    ...
    BOOL didPlay = NO;
    BOOL shouldSendTouchesToView = YES;
    ...
    for (UIGestureRecognizer* gr in touch.view.gestureRecognizers) {
        if ([gr isKindOfClass:objc_getClass("UIWebTouchEventsGestureRecognizer")]) {
            ...
            int eventTypeTouchesBegin = [MTUtils isOs7Up] ? 0 : 6;
            [webRecognizer _processTouches:[eventDown allTouches] withEvent:eventDown type:eventTypeTouchesBegin];
            [touch setPhase:UITouchPhaseEnded];
            
            UIEvent *eventUp = [[UIEvent alloc] initWithTouch:touch];
            int eventTypeTouchesEnd = [MTUtils isOs7Up] ? 2 : 8;
            [webRecognizer _processTouches:[eventUp allTouches] withEvent:eventUp type:eventTypeTouchesEnd];
            
            didPlay = YES;
            if (gr.cancelsTouchesInView) {
                //[eventDown release];
                shouldSendTouchesToView=NO;
            }
        }
    ...
    if (shouldSendTouchesToView) {
        if ([touch.view shouldCallResponderTouches]) {
...
//	[touch setPhase:UITouchPhaseEnded];
    ...
    //	//[[UIApplication sharedApplication] sendEvent:eventUp];
    //    [touchEnd setPhase:UITouchPhaseEnded];
