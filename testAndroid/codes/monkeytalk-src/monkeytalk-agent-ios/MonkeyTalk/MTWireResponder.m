...
+ (NSObject<MTHTTPResponse> *) wireResponseFromQuery:(NSString *)query
    ...
    if(data == nil) {
        NSString *responseString = [NSString stringWithFormat:@"<html><head><title>MonkeyTalk</title></head><body><h1>OK</h1><p>server running on port %@</p><p>%@</p></body></html>", MT_PLAYBACK_PORT, [MTBuildStamp buildStamp]];
        return [[MTHTTPDataResponse alloc] initWithData:[responseString dataUsingEncoding:NSUTF8StringEncoding]];
    }
...
+ (NSObject<MTHTTPResponse> *)screenshotResponse {
    ...
    UIImage *screenshot = [MTUtils screenshot];
    ...
    if (filePath && [[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir) {
        return [[MTHTTPFileResponse alloc] initWithFilePath:filePath forConnection:nil];
