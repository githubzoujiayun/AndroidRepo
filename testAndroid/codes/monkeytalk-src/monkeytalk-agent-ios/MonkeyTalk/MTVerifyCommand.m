...
+ (void) handleVerify:(MTCommandEvent*) ev {
    [self performSelectorOnMainThread:@selector(handleVerifyOnMainThread:) withObject:ev waitUntilDone:YES];
}

+ (void)handleVerifyOnMainThread:(MTCommandEvent *)ev {
...
+ (void)verifyPropValue:(NSString *)propValue event:(MTCommandEvent *)ev isVerifyNot:(BOOL)isVerifyNot isWildCard:(BOOL)isWildCard isRegEx:(BOOL)isRegEx regex:(NSRegularExpression *)regex {
    if (isRegEx) {
...
+ (NSString*) execute:(MTCommandEvent*) ev isVerifyNot:(BOOL)isVerifyNot isRegEx:(BOOL)isRegEx isWildCard:(BOOL)isWildCard {
    ...
    BOOL isWildcardMonkeyId = [ev.monkeyID rangeOfString:@"*"].location != NSNotFound ||
    ...
    if (!ev.value)
        source = ev.source;
	if (source == nil && !ev.value && !isWildcardMonkeyId) {
	...
	if ([ev.args count] > 0) {
        ...
        if (isWildcardMonkeyId) {
            ...
            if ([ev.component.lowercaseString isEqualToString:MTComponentDevice.lowercaseString]) {
                propValue = [MTDevice foundValue:ev.args];
                [self verifyPropValue:propValue event:ev isVerifyNot:isVerifyNot isWildCard:isWildCard isRegEx:isRegEx regex:regex];
            } else {
            ...
            NSArray *webViews = [NSClassFromString(@"UIWebView") orderedViewsWithWildcardMonkeyId:ev.monkeyID];
            
            for (NSValue *value in webViews) {
                UIWebView *webView = [value nonretainedObjectValue];
                
                if ([webView.delegate isKindOfClass:[MTWebViewController class]]) {
                    MTWebViewController *webController = (MTWebViewController *)webView.delegate;
                    NSArray *webValues = [webController valuesForEvent:ev property:prop];
                    
                    if (webValues.count == 0) {
                        NSString *result = [NSString
                                            stringWithFormat: @"No %@ found with monkeyID \"%@\"",
                                            ev.component, ev.monkeyID];
                        ev.lastResult = ev.lastResult ? [ev.lastResult stringByAppendingFormat:@" or \"\""] : result;
                        return ev.lastResult;
                    }
                    
                    for (NSString *propValue in webValues) {
                        [self verifyPropValue:propValue event:ev isVerifyNot:isVerifyNot isWildCard:isWildCard isRegEx:isRegEx regex:regex];
