...
@interface MTDDLogFileManagerDefault (PrivateAPI)
...
@interface MTDDFileLogger (PrivateAPI)
...
@implementation MTDDLogFileManagerDefault
                       ...
                       context:(void *)context
{
	...
	if ([keyPath isEqualToString:@"maximumNumberOfLogFiles"])
	{
		...
		dispatch_async([MTDDLog loggingQueue], ^{ @autoreleasepool {
...
- (void)deleteOldLogFiles
{
	NSLogVerbose(@"MTDDLogFileManagerDefault: deleteOldLogFiles");
	...
	if (count > 0)
	{
		MTDDLogFileInfo *logFileInfo = [sortedLogFileInfos objectAtIndex:0];
	...
	{
		MTDDLogFileInfo *logFileInfo = [sortedArchivedLogFileInfos objectAtIndex:i];
		
		NSLogInfo(@"MTDDLogFileManagerDefault: Deleting file: %@", logFileInfo.fileName);
...
/**
 * Returns an array of MTDDLogFileInfo objects,
...
- (NSArray *)unsortedLogFileInfos
{
	...
	for (NSString *filePath in unsortedLogFilePaths)
	{
		MTDDLogFileInfo *logFileInfo = [[MTDDLogFileInfo alloc] initWithFilePath:filePath];
...
- (NSArray *)sortedLogFilePaths
{
	...
	for (MTDDLogFileInfo *logFileInfo in sortedLogFileInfos)
	{
...
- (NSString *)createNewLogFile
{
	...
	do
	{
		...
		{
			NSLogVerbose(@"MTDDLogFileManagerDefault: Creating new log file: %@", fileName);
...
@implementation MTDDLogFileFormatterDefault
...
@implementation MTDDFileLogger

- (id)init
{
	MTDDLogFileManagerDefault *defaultLogFileManager = [[MTDDLogFileManagerDefault alloc] init];
	
	return [self initWithLogFileManager:defaultLogFileManager];
}

- (id)initWithLogFileManager:(id <MTDDLogFileManager>)aLogFileManager
{
	if ((self = [super init]))
	{
		...
		formatter = [[MTDDLogFileFormatterDefault alloc] init];
	}
	return self;
}
...
**/
- (MTDDLogFileInfo *)currentLogFileInfo
{
	if (currentLogFileInfo == nil)
	{
		...
		{
			MTDDLogFileInfo *mostRecentLogFileInfo = [sortedLogFileInfos objectAtIndex:0];
		...
		if (currentLogFileInfo == nil)
		{
			...
			currentLogFileInfo = [[MTDDLogFileInfo alloc] initWithFilePath:currentLogFilePath];
		}
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark MTDDLogger Protocol
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
@implementation MTDDLogFileInfo
...
{
	return [[MTDDLogFileInfo alloc] initWithFilePath:aFilePath];
}
...
- (NSDate *)creationDate
{
	if (creationDate == nil)
	{
		...
		{
			NSLogError(@"MTDDLogFileInfo: creationDate(%@): getattrlist result = %i", self.fileName, result);
		}
...
- (void)renameFile:(NSString *)newFileName
{
	...
	if (![newFileName isEqualToString:[self fileName]])
	{
		...
		NSLogVerbose(@"MTDDLogFileInfo: Renaming file: '%@' -> '%@'", self.fileName, newFileName);
		
		NSError *error = nil;
		if (![[NSFileManager defaultManager] moveItemAtPath:filePath toPath:newFilePath error:&error])
		{
			NSLogError(@"MTDDLogFileInfo: Error renaming file (%@): %@", self.fileName, error);
		}
...
- (void)addExtendedAttributeWithName:(NSString *)attrName
{
	...
	if (result < 0)
	{
		NSLogError(@"MTDDLogFileInfo: setxattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName
{
	...
	{
		NSLogError(@"MTDDLogFileInfo: removexattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
...
- (BOOL)isEqual:(id)object
{
	...
	{
		MTDDLogFileInfo *another = (MTDDLogFileInfo *)object;
...
- (NSComparisonResult)reverseCompareByCreationDate:(MTDDLogFileInfo *)another
{
...
- (NSComparisonResult)reverseCompareByModificationDate:(MTDDLogFileInfo *)another
{
