...
@interface MTHTTPServer (PrivateAPI)
...
- (void)stop:(BOOL)keepExistingConnections
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		if (!keepExistingConnections)
		{
			...
			for (MTHTTPConnection *connection in connections)
			{
				[connection stop];
			}
			...
			[webSocketsLock lock];
			for (MTWebSocket *webSocket in webSockets)
			{
				[webSocket stop];
			}
...
- (void)addWebSocket:(MTWebSocket *)ws
{
	[webSocketsLock lock];
...
- (MTHTTPConfig *)config
{
	...
	return [[MTHTTPConfig alloc] initWithServer:self documentRoot:documentRoot queue:connectionQueue];
}

- (void)socket:(MTGCDAsyncSocket *)sock didAcceptNewSocket:(MTGCDAsyncSocket *)newSocket
{
	MTHTTPConnection *newConnection = (MTHTTPConnection *)[[connectionClass alloc] initWithAsyncSocket:newSocket
