...
// Note that the recording functionality is actually implemented in UIScrollView (MTReady). This is
// because we're not sure
// about overriding the setContentOffset method here, since we swizzle it in UIScrollView. The
// Objective-C doc is unclear
// on the safety of overriding a swizzled method (and it's not clear there's a prescribed order of
// class loading so
// further swizzling the swizzled methods here seems dangerous. As a result, we do an explicit type
// check in
// UIScrollView to see if we're actually a UITableView, and if we are, we record the (section,row)
// tuple rather than (x,y)

@interface UITableView (Intercept)
- (void)originalTableView:(UITableView *)tableView
       moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath
              toIndexPath:(NSIndexPath *)destinationIndexPath;
- (void)origTableView:(UITableView *)tableView
    ...
    if ([prop isEqualToString:@"selected" ignoreCase:YES]) {
        ...
        if (indexPath) {
            if ([self numberOfSections] == 1) {
                ...
                result =
                    [NSString stringWithFormat:@"%d,%d", indexPath.row + 1, indexPath.section + 1];
            }
        } else {
            result = @"No Row Selected";
        }
        ...
        if (shouldReturnArray) {
            ...
            for (int j = 0; j < [self numberOfSections]; j++) {
                ...
                for (int i = 0; i < [self numberOfRowsInSection:j]; i++) {
                    [self scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:j]
                                atScrollPosition:UITableViewScrollPositionNone
                                        animated:NO];
                    UITableViewCell *temp = [self
                        cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:section]];
            ...
            UITableViewCell *cell =
                [self cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:section]];
        ...
        [self scrollToRowAtIndexPath:startIndex
                    atScrollPosition:UITableViewScrollPositionNone
                            animated:NO];
    } else if ([prop isEqualToString:@"size" ignoreCase:YES]) {
        result = [NSString stringWithFormat:@"%i", [self numberOfRowsInSection:arg1Int]];
    } else {
        NSIndexPath *indexPath = [self.class indexPathFromProp:prop];
        if (indexPath) {
            // ios only:
            // Table * get myVar some.prop.path.on.cell(row)
            // Table * get myVar some.prop.path.on.cell(row,section)
            NSString *base = [self.class baseFromProp:prop];
            UITableViewCell *cell = [self cellForRowAtIndexPath:indexPath];
            result = [cell valueForKeyPath:base];
        } else {
...
// TODO: refactor value property code to better handle properties with parenths
+ (NSIndexPath *)indexPathFromProp:(NSString *)prop {
    NSArray *argArray = [self argArrayFromProp:prop];

    if (!argArray) {
        return nil;
    }

    NSInteger row = [[argArray objectAtIndex:0] integerValue] - 1;
    NSInteger section = argArray.count > 1 ? [[argArray objectAtIndex:1] integerValue] - 1 : 0;

    return [NSIndexPath indexPathForRow:row inSection:section];
}

+ (NSString *)baseFromProp:(NSString *)prop {
    NSRange matchRange = [prop rangeOfString:@"("];
    NSString *base =
        matchRange.location != NSNotFound ? [prop substringToIndex:matchRange.location] : prop;

    return base;
}

+ (NSArray *)argArrayFromProp:(NSString *)prop {
    NSMutableArray *argArray = [NSMutableArray new];

    NSString *regexString = @"\\((\\d+)[,\\)]|(\\d+)[,\\)]";
    NSError *error = nil;
    NSRegularExpression *regex =
        [[NSRegularExpression alloc] initWithPattern:regexString
    ...
    NSArray *regexResults =
        [regex matchesInString:prop options:0 range:NSMakeRange(0, [prop length])];
    ...
    for (NSTextCheckingResult *b in regexResults) {
        NSString *foundRegex = [prop substringWithRange:b.range];
        foundRegex = [foundRegex stringByReplacingOccurrencesOfString:@"(" withString:@""];
        foundRegex = [foundRegex stringByReplacingOccurrencesOfString:@"," withString:@""];
        foundRegex = [foundRegex stringByReplacingOccurrencesOfString:@")" withString:@""];
        [argArray addObject:foundRegex];
    }
...
- (void)mtSetDataSource:(NSObject<UITableViewDataSource> *)del {
    // The existence of this method triggers swipe-to-edit functionality in a UITableView (and who
    // knows what else).
    // If we add this method and it doesn't already exist, we inadvertently enable swipe-to-edit
    // So, if this method doesn't already exist, we assume the table is not editable and therefore
    // we don't have to record these delegate calls.
    if ([del mtHasMethod:@selector(tableView:commitEditingStyle:forRowAtIndexPath:)]) {
        ...
        if ([del mtHasMethod:@selector(tableView:moveRowAtIndexPath:toIndexPath:)]) {
    ...
    //	Method originalMethod = class_getInstanceMethod([del class],
    //@selector(tableView:commitEditingStyle:forRowAtIndexPath:));
    //	if (originalMethod) {
    //		IMP origImp = method_getImplementation(originalMethod);
    //		Method replacedMethod = class_getInstanceMethod([self class],
    //@selector(mtTableView:commitEditingStyle:forRowAtIndexPath:));
    //		IMP replImp = method_getImplementation(replacedMethod);
    //
    //		if (origImp != replImp) {
    //			method_setImplementation(originalMethod, replImp);
    //			class_addMethod([del class],
    //@selector(origTableView:commitEditingStyle:forRowAtIndexPath:), origImp,"v@:@i@");
    //		}
    //	}
...
//    }
//
//    [MonkeyTalk recordFrom:firstTable command:MTCommandSetEditing args:[NSArray arrayWithObjects:
// editingString, nil]];
//
//    [self origSetEditing:editing animated:animated];
//}

- (void)mtTableView:(UITableView *)tableView
    moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath
           toIndexPath:(NSIndexPath *)destinationIndexPath {

    if (sourceIndexPath != destinationIndexPath)
        [MonkeyTalk
            recordFrom:tableView
               command:MTCommandMove
                  args:[NSArray arrayWithObjects:
                                    [NSString stringWithFormat:@"%d", sourceIndexPath.row + 1],
                                    [NSString stringWithFormat:@"%d", destinationIndexPath.row + 1],
                                    nil]];

    [self originalTableView:tableView
...
- (void)mtTableView:(UITableView *)tableView
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        [MonkeyTalk
            recordFrom:tableView
               command:MTCommandRemove
                  args:[NSArray arrayWithObjects:
                                    [NSString stringWithFormat:@"%d", indexPath.row + 1],
                                    [NSString stringWithFormat:@"%d", indexPath.section + 1], nil]];
    } else if (editingStyle == UITableViewCellEditingStyleInsert) {
        [MonkeyTalk
            recordFrom:tableView
               command:MTCommandInsert
                  args:[NSArray arrayWithObjects:
                                    [NSString stringWithFormat:@"%d", indexPath.row + 1],
                                    [NSString stringWithFormat:@"%d", indexPath.section + 1], nil]];
    }
...
- (void)mtTableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [MonkeyTalk recordFrom:tableView command:MTCommandSelectIndex args:argsArray];
...
- (void)playbackMonkeyEvent:(MTCommandEvent *)event {
    ...
    if ([[event args] count] == 0) {
        event.lastResult = [NSString
            stringWithFormat:@"Requires 1 or more arguments, but has %d", [event.args count]];
        return;
    }
    ...
    if ([event.command isEqualToString:MTCommandVScroll] ||
        ...
        //			event.lastResult = @"Requires 0, 1, or 2 arguments, but has %d",
        //[event.args count];
        //		}
        ...
        NSIndexPath *path = [self indexPathTextValueInEvent:event];
        ...
        if (path == nil) {
            event.lastResult =
                [NSString stringWithFormat:@"Could not find cell %@ in table with monkeyID %@",
                                           [event.args objectAtIndex:0], event.monkeyID];
            return;
        } else if (row + 1 > [self numberOfRowsInSection:section]) {
            event.lastResult = [NSString
                stringWithFormat:@"Scroll out of bounds -- can't scroll to row %i, because section "
                                 @"%i only has %i %@",
                                 row + 1, section + 1, [self numberOfRowsInSection:section],
                                 [NSString pluralStringFor:@"row"
                                                 withCount:[self numberOfRowsInSection:section]]];
            return;
        } else if (section + 1 > [self numberOfSections]) {
            event.lastResult =
                [NSString stringWithFormat:@"Scroll out of bounds -- can't scroll to row %i in "
                                           @"section %i, because table only has %i %@",
                                           row + 1, section + 1, [self numberOfSections],
                                           [NSString pluralStringFor:@"section"
                                                           withCount:[self numberOfSections]]];
            return;
        }
        // UITableViewScrollPositionNone changed from UITableViewScrollPositionTop to resolve two
        // bugs --KplMax
        ...
        if (row + 1 > [self numberOfRowsInSection:section]) {
            event.lastResult = [NSString
                                stringWithFormat:@"Delete out of bounds -- can't scroll to row %i, because section "
                                @"%i only has %i %@",
                                row + 1, section + 1, [self numberOfRowsInSection:section],
                                [NSString pluralStringFor:@"row"
                                                withCount:[self numberOfRowsInSection:section]]];
        ...
        if ([(UITableView *)self.dataSource
                respondsToSelector:@selector(origTableView:commitEditingStyle:forRowAtIndexPath:)])
            [(UITableView *)self.dataSource origTableView:self
        ...
        if ([event.command isEqualToString:MTCommandSelect ignoreCase:YES]) {
            indexPath = [self indexPathTextValueInEvent:event];
        ...
        if (indexPath == nil) {
            ...
            event.lastResult = [NSString
                stringWithFormat:@"Selection out of bounds -- can't select row %i, because section "
                                 @"%i only has %i %@",
                                 row + 1, section + 1, [self numberOfRowsInSection:section],
                                 [NSString pluralStringFor:@"row"
                                                 withCount:[self numberOfRowsInSection:section]]];
            ...
            event.lastResult =
                [NSString stringWithFormat:@"Selection out of bounds -- can't select row %i in "
                                           @"section %i, because table only has %i %@",
                                           row + 1, section + 1, [self numberOfSections],
                                           [NSString pluralStringFor:@"section"
                                                           withCount:[self numberOfSections]]];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([event.command isEqualToString:MTCommandSelectIndicator ignoreCase:YES]) {
                    ...
                    if ([self.delegate
                        ...
                        UIViewController *vc = [self respondsToSelector:@selector(delegate)] ? (UIViewController *)self.delegate : nil;
        ...
        if ([[event args] count] < 2) {
            event.lastResult =
                [NSString stringWithFormat:@"Requires 2 arguments, but has %d", [event.args count]];
            return;
        }

        NSInteger sourceRow = [event.args count] > 0 ? [[event.args objectAtIndex:0] intValue] : 0;
        NSInteger destinationRow =
            [event.args count] > 1 ? [[event.args objectAtIndex:1] intValue] : 0;
        ...
        [(UITableView *)self.delegate originalTableView:self
                                     moveRowAtIndexPath:sourceIndexPath
                                            toIndexPath:destinationIndexPath];
...
- (NSIndexPath *)indexPathTextValueInEvent:(MTCommandEvent *)event {
    NSString *title = [event.args objectAtIndex:0];
    NSString *propPath = event.args.count > 1 ? [event.args objectAtIndex:1] : @".textLabel.text";
    
    if ([propPath rangeOfString:@"."].location != 0 && [event.command isEqualToString:MTCommandSelect ignoreCase:YES]) {
        // requires . prefix
        event.lastResult = [NSString stringWithFormat:@"Select property path arg must include a \".\" prefix i.e. \".%@\"",propPath];
        return nil;
    } else {
        propPath = [propPath substringFromIndex:1];
    }
    ...
    for (int i = 0; i < [self numberOfSections]; i++) {
        for (int j = 0; j < [self numberOfRowsInSection:i]; j++) {
            ...
            // UIMoreListController (more tab) does not return correct cell from
            // tableView:cellForRowAtIndexPath:
            if ([self.dataSource
                    respondsToSelector:@selector(tableView:cellForRowAtIndexPath:)] &&
                ![self.dataSource isKindOfClass:NSClassFromString(@"UIMoreListController")]) {
                cell = [self.dataSource tableView:self cellForRowAtIndexPath:currentPath];
            } else {
                cell = [self cellForRowAtIndexPath:currentPath];
            }
            ...
            @try {
                if ([[cell valueForKeyPath:propPath] isEqualToString:title]) {
...
+ (NSString *)uiAutomationCommand:(MTCommandEvent *)command {
    ...
    if ([command.command isEqualToString:MTCommandVScroll]) {
        ...
        [string appendFormat:@"MonkeyTalk.scrollTo(\"%@\", \"%@\", \"%@\")", command.monkeyID,
                             section, row];
    } else {
