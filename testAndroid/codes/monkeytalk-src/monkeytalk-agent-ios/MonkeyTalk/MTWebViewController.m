...
typedef enum { COMMAND, ID, FIELD, ARGS } fields;
...
- (void)setUp {
    ...
    if ([[UIDevice currentDevice]
            respondsToSelector:@selector(setMediaPlaybackRequiresUserAction)]) {
...
- (BOOL)webView:(UIWebView *)webView2
    ...
    // Handling Record event for web components; receives key and values from java script, parse the
    // values according to MonkeyTalk API
    ...
    if ([requestArray count] > 1) {
        ...
        if ([delController
            ...
            return [delController webView:webView2
...
- (void)webViewDidStartLoad:(UIWebView *)webView2 {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"webdriver:pageLoad" object:self];
    @synchronized(self) {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView2 {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"monkeytalk-custom-web" ofType:@"js"];
    NSString *customContent = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
    [self.webView stringByEvaluatingJavaScriptFromString:customContent];
...
- (void)webView:(UIWebView *)webView2 didFailLoadWithError:(NSError *)error {
    ...
    if ([error code] == 101) {
        ...
        if ([[failingURLString substringToIndex:4] isEqualToString:@"tel:"]) {
            @throw [NSException
                webDriverExceptionWithMessage:
                    [NSString stringWithFormat:@"tel: url isn't supported in simulator"]
...
- (void)performSelectorOnView:(SEL)selector withObject:(id)value waitUntilLoad:(BOOL)wait {
    ...
    [[self webView] performSelectorOnMainThread:selector withObject:value waitUntilDone:YES];
...
- (void)setURL:(NSDictionary *)urlMap {
    ...
    [self performSelectorOnView:@selector(loadRequest:) withObject:url waitUntilLoad:YES];
}

- (void)back:(NSDictionary *)ignored {
    [self describeLastAction:@"back"];
    [self performSelectorOnView:@selector(goBack) withObject:nil waitUntilLoad:YES];
}

- (void)forward:(NSDictionary *)ignored {
    [self describeLastAction:@"forward"];
    [self performSelectorOnView:@selector(goForward) withObject:nil waitUntilLoad:YES];
}

- (void)refresh:(NSDictionary *)ignored {
    [self describeLastAction:@"refresh"];
    [self performSelectorOnView:@selector(reload) withObject:nil waitUntilLoad:YES];
}
...
- (void)jsEvalInternal:(NSString *)script {
    ...
    [CATransaction begin];
    lastJSResult_ = nil;
    lastJSResult_ = [[self webView] stringByEvaluatingJavaScriptFromString:script];
...
- (NSString *)jsEval:(NSString *)format, ... {
    ...
    NSString *script = [[NSString alloc] initWithFormat:format arguments:argList];
    va_end(argList);
...
- (NSString *)evalFor:(int)frame jsEval:(NSString *)format, ... {
    ...
    [self
        performSelectorOnMainThread:@selector(jsEvalInternal:)
                         withObject:[NSString stringWithFormat:@"(function(){var w=window;var "
                                                               @"frameIndexes=[\"%i\"];for(var "
                                                               @"i=0;i<frameIndexes.length;i++){"
                                                               @"if(!(w=w.frames[frameIndexes[i]])"
                                                               @")return true;};return false;})()",
                                                               frame]
                      waitUntilDone:YES];
    if (![[lastJSResult_ copy] isEqualToString:@"true"]) {
        script = [[NSString alloc]
            initWithFormat:@"(function(){var win=(function(){var w=window;var "
                           @"frameIndexes=[\"%i\"];for(var "
                           @"i=0;i<frameIndexes.length;i++){if(!(w=w.frames[frameIndexes[i]]))"
                           @"return window;};return w;})(); return win.eval('%@');})()",
                           frame,
...
- (void)scrollIntoView:(CGPoint)point {
    ...
    [self jsEval:@"window.scroll(%f - window.innerWidth / 2, %f - window.innerHeight / 2);",
                 point.x, point.y];
}
...
- (void)performSelectorOnElement:(MTElement *)element
    ...
    [element performSelectorOnMainThread:selector withObject:value waitUntilDone:YES];
...
- (void)runCommandAtIndex:(int)i {
    [self performSelectorInBackground:@selector(performInBack:)
...
- (void)throwScriptFailed:(NSString *)failureString {
    ...
    commandIndex = (int)[commandList count];
}

- (void)verifyWebElement:(MTElement *)element forCommand:(MTCommandEvent *)event {
    NSString *mtComponent =
        [MTConvertType convertedComponentFromString:event.className isRecording:YES];
...
- (NSInteger)updateOrdinalForTag:(NSString *)htmlTag {
    ...
    if ([htmlTag isEqualToString:MTComponentButton ignoreCase:YES]) {
        count += [[webView stringByEvaluatingJavaScriptFromString:
                               [NSString stringWithFormat:@"var count = 0; var inputs = "
                                         @"document.getElementsByTagName('input'); for (var i = "
                                         @"0; i < inputs.length; i++) {var input = inputs[i]; "
                                         @"if (input.getAttribute('type') == 'submit' || "
                                         @"input.getAttribute('type') == 'reset') {count++;} "
                                         @"return count.toString();}"]] intValue];
    }

    return count;
}

- (NSArray *)valuesForEvent:(MTCommandEvent *)event property:(NSString *)prop {
    NSMutableArray *values = [[NSMutableArray alloc] init];
    MTSeleniumCommand *selCommand = [MTSeleniumCommand initWithMTCommandEvent:event];
    NSString *xpath = selCommand.xpathBase;
    xpath = [xpath stringByReplacingOccurrencesOfString:@"'" withString:@"\""];
    
    MTHTTPVirtualDirectory *virtualDirectory = [[MTHTTPVirtualDirectory alloc] init];
    NSString *js = [NSString stringWithFormat:@"return MonkeyTalk.getElementValuesFromXpath('%@');",xpath];
    NSString *jsResult = [virtualDirectory executeMonkeyTalkJs:js];
    
    NSArray *result = [NSJSONSerialization JSONObjectWithData:[jsResult dataUsingEncoding:NSUTF8StringEncoding] options:0 error:nil];
    if (result) {
        [values addObjectsFromArray:result];
    }
...
- (BOOL)playBackCommandInWebView:(MTCommandEvent *)event {
    if ([MonkeyTalk sharedMonkey].currentCommand.found || event.didPlayInWeb)
        return nil;

    BOOL isVerifyNotExistance =
        [event.command.lowercaseString rangeOfString:@"verifynot"].location != NSNotFound &&
        event.args.count == 0;
    MTSeleniumCommand *selCommand = [MTSeleniumCommand initWithMTCommandEvent:event];
    ...
    if (selCommand.element) {
        ...
        selCommand.element.webView = self.webView;
        didPlay = YES;
        ...
        if (selCommand.mtEvent.value && !event.lastResult) {
            if ([selCommand.mtEvent.command.lowercaseString
                    rangeOfString:MTCommandVerify.lowercaseString].location != NSNotFound) {
    ...
    if ([MTUtils isOs7Up]) {
        ...
        while (nextResponder) {
            if ([nextResponder isKindOfClass:[UINavigationController class]]) {
                transformedPoint.y -= nextResponder.navigationBar.frame.size.height +
...
- (void)highlightElement:(MTElement *)element {
    ...
    UIView *touchView =
        [[UIView alloc] initWithFrame:CGRectMake(location.x, location.y, size.width, size.height)];
    touchView.backgroundColor = [UIColor clearColor];
    touchView.layer.borderColor =
        [UIColor colorWithRed:6 / 255.0 green:102 / 255.0 blue:214 / 255.0 alpha:1].CGColor;
...
- (NSString *)webComponentTree {
    if (!webComponentTree) {
        ...
        [webView stringByEvaluatingJavaScriptFromString:
                 ...
                 @"iframe.setAttribute(\"src\", \"monkeytalk:loadNext\"); "
