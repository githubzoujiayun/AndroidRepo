...
- (void)interceptMethod:(SEL)orig withClass:(Class)class types:(char *)types {
    @autoreleasepool {
        ...
        SEL alias = NSSelectorFromString([NSString stringWithFormat:@"mt_%s", origName]);
        Method replaceMethod = class_getInstanceMethod(class, alias);
        IMP replImp = method_getImplementation(replaceMethod);

        if (origImp != replImp) {
            if (originalMethod) {
                if (origImp == _objc_msgForward) {
                    id block = ^BOOL (id self, SEL selector) {
                        return objc_getAssociatedObject(self, alias);
                    };

                    class_replaceMethod(class, orig, imp_implementationWithBlock(block), types);
                    return;
                }
