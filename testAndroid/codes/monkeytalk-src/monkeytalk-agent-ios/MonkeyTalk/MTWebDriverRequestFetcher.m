...
-(void)parseWebDriverRequest:(NSDictionary*)wdRequest
  ...
  *methodRef = [[wdRequest objectForKey:@"Method"] objectAtIndex:0];
  ...
  NSString *path = [[[NSURL alloc] initWithString:urlStr] path];
  NSString *connPath = [[[NSURL alloc] initWithString:connectorAddr] path];
...
-(id<MTHTTPResponse,NSObject>)httpResponseForQuery:(NSString *)query
  ...
  CFHTTPMessageRef message = CFHTTPMessageCreateRequest(
      kCFAllocatorDefault, (__bridge CFStringRef)method, cfURL, kCFHTTPVersion1_1);
...
+(NSDictionary*) sendResponse:(id<MTHTTPResponse,NSObject>) response
  ...
  if (response == nil) {
    ...
    cmdRespStatus = @"302";
    [cmdRespHeaders setValue:@"text/plain" forKey:@"Content-Type"];
    NSString* redirectedDest = [(MTHTTPRedirectResponse* )response performSelector:@selector(destination)];
...
- (void) fetchAndProcessRequests{
    @autoreleasepool {
        ...
        NSURL* urlFetchRequest = [[NSURL alloc] initWithString:urlString];
        ...
        NSURL* urlSendResponse = [[NSURL alloc] initWithString:urlString];
