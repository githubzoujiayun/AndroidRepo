...
- (void)recordCollectionViewScrollToIndex {
    UICollectionView* collection = (UICollectionView*) self;
    NSArray* cells = [collection visibleCells];
    ...
    if (self.contentOffset.y >= 0) {
        if ([cells count]) {
            ...
            for (UICollectionViewCell *cell in cells) {
                ...
                if (path.item < item || item == -1) {
                    item = path.item;
                }
                
                if (path.section < section || section == -1) {
                    section = path.section;
                }
            }
            
            NSString *rowArg = [NSString stringWithFormat:@"%i", (int)item+1];
            NSString *sectionArg = section > 0 ? [NSString stringWithFormat:@"%i", (int)section+1] : nil;
            ...
            NSArray *recordArray = [NSArray
             ...
             [[MonkeyTalk sharedMonkey] postCommandFrom:self
                                               command:MTCommandScrollToIndex
                                                  args:recordArray];
        }
    }
}
...
- (void)recordScroll {
    ...
    else if ([self isKindOfClass:[UICollectionView class]] && self.contentOffset.y >= 0) {
        [self recordCollectionViewScrollToIndex];
    } else {
