...
+ (UIView *)findViewFromEvent:(MTCommandEvent *)event {
    ...
    BOOL shouldUpdateMonkeyId = [MTUtils isMinimumVersion:@"8.0"] && findClass == [UITableView class] && [event.monkeyID rangeOfString:@"Empty list"].location != NSNotFound;
    
    if (shouldUpdateMonkeyId) {
        // Table view accessibility label no longer defaults to Empty list on iOS 8
        event.monkeyID = @"*";
    }
