...
public class CommandWorld {
	...
	/**
	 * Cache of where unzipped mtl files are stored
	 */
	...
	/**
	 * Lists all the files in the command world. This will contain all the files in the rootDir as
	 * well as any files contained in MTL files. Additionally, if any MTL files are found that are
	 * not currently unzipped, it will create those caches
	 * 
	 * @return list of all files in the command world
	 */
	private File[] listAllFiles() {
		if (rootDir != null && rootDir.isDirectory()) {
			for (File f : rootDir.listFiles()) {
				// if there's an MTL we haven't seen yet, unzip it
				if (f.getName().endsWith(ZIP_EXT) && !libraryCache.containsKey(f.getName())) {
					unzipMtl(f);
				}
			}
			File[] fileList = rootDir.listFiles();
			// for every mtl file we've already unzipped, add files to the list
			for (File mtl : libraryCache.values()) {
				if (mtl != null && mtl.isDirectory()) {
					fileList = (File[]) ArrayUtils.addAll(fileList, mtl.listFiles());
				}
			}
			return fileList;
		} else {
			// root dir not set up correctly - no files
			return new File[0];
		}
	}

	/**
	 * unzip the MTL file to a temp folder, and add it to the MTL cache so that it will be indexed
	 * 
	 * @param mtl
	 *            the .mtl file to unzip
	 */
	private void unzipMtl(File mtl) {
		try {
			File unzipTemp = FileUtils.tempDir();
			FileUtils.unzipFile(mtl, unzipTemp);
			libraryCache.put(mtl.getName(), unzipTemp);
		} catch (Exception e) {
			System.out.println("Failed to unzip " + mtl.getName()
