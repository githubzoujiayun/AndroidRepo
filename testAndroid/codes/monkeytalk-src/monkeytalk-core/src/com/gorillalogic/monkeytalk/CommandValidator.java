...
/**
 * Class returned from the command validation process with a status and a message.
 */
public class CommandValidator {
	private CommandStatus status;
	private String message;
	private static final Set<String> validModifiers = new HashSet<String>();
	static {
		validModifiers.add(Command.ABORT_MODIFIER);
	...
	/**
	 * Validate the given command and return a new {@link CommandValidator} object with
	 * {@link CommandStatus#OK} if valid, otherwise return a non-OK status plus a detailed error
	 * message.
	 * 
	...
	public static CommandValidator validate(Command cmd) {
		...
		if (cmd.getComponentType() == null || cmd.getComponentType().length() == 0) {
		...
		for (String modifier : cmd.getModifiers().keySet()) {
			if (!validModifiers.contains(modifier)) {
				return new CommandValidator(CommandStatus.BAD_MODIFIERS, "unknown modifier \""
						+ modifier + "\"");
			}
		}
