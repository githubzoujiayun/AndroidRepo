		...
		if (cmd.getArgs().size() == 0) {
			...
			for (int i = 0; i < cmd.getArgs().size(); i++) {
				...
				if (arg.contains("=")) {
					...
					// in the special case of a local var having no default value, if there is an
					// existing global variable of the same name, use it's value instead
					if (Globals.hasGlobal(key)) {
						val = Globals.getGlobal(key);
						usingGlobal = true;
					}
				}
				...
				if (scope.getVariables().containsKey(key)) {
					...
					if (parentArg.equals("*")) {
						...
						if (usingGlobal) {
							result.setWarning((result.getWarning() != null ? result.getWarning()
									+ ", " : "")
									+ "using global value '" + val + "' for variable '" + key + "'");
						}
					} else {
					...
					// not data-driving, and no parent command args, so just use default value from
					// the vars.define command
					scope.addVariable(key, val);
					if (usingGlobal) {
						result.setWarning((result.getWarning() != null ? result.getWarning() + ", "
								: "")
