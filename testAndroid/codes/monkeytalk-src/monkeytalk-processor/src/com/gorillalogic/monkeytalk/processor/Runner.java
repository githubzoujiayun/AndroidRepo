...
public class Runner {
	...
	public PlaybackResult run(File in, Map<String, String> globals) {
		...
		System.out.println(BuildStamp.getStamp());
		...
		if (in == null) {
			...
			try {
				initGlobals(in.getAbsoluteFile().getParentFile(), globals);
	 ...
	 */
	public void initGlobals(File dir, Map<String, String> globals) throws IOException {
		Globals.clear();
		if (dir != null && dir.exists() && dir.isDirectory()) {
			File props = new File(dir, "globals.properties");
			if (props != null && props.exists() && props.isFile()) {
				System.out.println("loading globals.properties");
				Globals.setGlobals(props);
			}
		}
		if (globals != null && globals.size() > 0) {
			...
			System.out.println("loading globals: " + sb.toString());
	...
	protected void writeDetailReport(PlaybackResult result, Scope scope, File reportDir,
			File projectDir) {
		new DetailReportWriter().writeDetailReport(result, scope, projectDir, reportDir, 
				getRunnerVersion(), getAgentVersion());
	}

	
	protected String getAgentVersion() {
		return getAgent().getName() + " v" + getAgent().getAgentVersion();
	}

	protected String getRunnerVersion() {
		return BuildStamp.getVersion(this.getClass().getSimpleName());
	}
	...
	private ScriptProcessor getScriptProcessor(File dir) {
		scriptProcessor = ScriptProcessorFactory.createScriptProcessor(dir, getAgent());
