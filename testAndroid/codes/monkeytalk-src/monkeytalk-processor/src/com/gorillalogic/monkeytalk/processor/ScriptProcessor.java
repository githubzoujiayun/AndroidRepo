...
import com.gorillalogic.monkeytalk.processor.command.ScriptRunIf;
...
public class ScriptProcessor extends BaseProcessor {
	...
	/**
	 * Instantiate a script processor with the given host, port, and project root directory. Not
	 * intended for public use - package unit tests only
	 * 
	 * @param host
	 *            the target host
	 * @param port
	...
	/**
	 * Not intended for public use - ScriptProcessorFactory only
	 * 
	 * @param rootDir
	 * @param agent
	 */
	...
	public PlaybackResult runScript(String filename, Scope scope) {
		...
		if (world.hasJavascriptOverride(filename)) {
			...
			PlaybackResult result = getJSProcessor().runJavascript(cmd, scope);
	...
	public PlaybackResult runScript(Command cmd, Scope scope) {
		...
		// init result
		PlaybackResult result = runCommand(cmd, scope);
	...
	protected PlaybackResult runCommand(Command cmd, Scope scope) {
		PlaybackResult result = null;

		CommandStatus status = CommandValidator.validate(cmd).getStatus();
		if (CommandStatus.OK != status) {
			return new PlaybackResult(PlaybackStatus.ERROR, status.toString());
		}
		...
		if (cmd.isIgnored()) {
			...
			result = new ScriptRunIf(cmd, scope, getPlaybackListener(), this).runIf();
		} else if ("script.runwith".equals(cmd.getCommandName())) {
			result = scriptRunWith(cmd, scope, result);
			...
			result = new Globals(cmd, scope, getPlaybackListener()).define();
				...
				|| "vars.set".equals(cmd.getCommandName())) {
			...
			getPlaybackListener().onComplete(scope, new Response());
		} else if ("debug.globals".equals(cmd.getCommandName())) {
			result = new Debug(cmd, scope, getPlaybackListener(), world.getRootDir().toString())
					.globals();
		} else if ("debug.vars".equals(cmd.getCommandName())) {
			result = new Debug(cmd, scope, getPlaybackListener(), world.getRootDir().toString())
					.vars();
		} else if ("debug.erase".equals(cmd.getCommandName())) {
			result = new Debug(cmd, scope, getPlaybackListener(), world.getRootDir().toString())
					.erase();
		} else if ("debug.print".equals(cmd.getCommandName())) {
			result = new Debug(cmd, scope, getPlaybackListener(), world.getRootDir().toString())
					.print();
		} else if ("debug.tree".equals(cmd.getCommandName())) {
			String filter = (cmd.getArgs().size() > 0 ? cmd.getArgs().get(0) : null);
			String format = (cmd.getArgs().size() > 1 ? cmd.getArgs().get(1) : Debug.TEXT);
			int timeout = cmd.getTimeout();
			result = new Debug(cmd, scope, getPlaybackListener(), world.getRootDir().toString(),
					getAgent().getCommandSender()).tree(filter, format, timeout);
		...
		} else if ("system.version".equals(cmd.getCommandName())) {
			return new Sys(cmd, scope, getPlaybackListener()).getMonkeyTalkVersion();
		} else if ("verifyImage".equalsIgnoreCase(cmd.getAction())) {
			VerifyImage verifyImage = new VerifyImage(cmd, scope, getPlaybackListener(), this,
					getWorld().getRootDir());
			if (cmd.getComponentType().equals("Device")) {
				verifyImage.setScreenshot(runCommand(SCREENSHOT_COMMAND).getImageFile());
			}
			result = verifyImage.verifyImage();
			...
			result = new WaitFor(cmd, scope, getPlaybackListener(), this).waitFor();
		} else {
	...
	private PlaybackResult scriptRunWith(Command cmd, Scope scope, PlaybackResult result) {
		if (cmd.getArgs().size() == 0) {
			result = new PlaybackResult(PlaybackStatus.ERROR, "command '" + cmd.getCommand()
			...
			if (data == null) {
				...
				for (Map<String, String> datum : data) {
					...
					steps.add(step);
					PlaybackResult r = runScript(cmd.getMonkeyId(), new Scope(cmd, scope, datum));
					step.setResult(r);
					getPlaybackListener().onComplete(scope, new Response());
	...
	public PlaybackResult playbackVanillaCommand(Command cmd, Scope scope) {
		getPlaybackListener().onStart(scope);
		File before = null;
		File after = null;
		String metrics = null;
