...
public class GlobalsTest {
	...
	@Test
	public void testSetWithIllegalVariableValue() {
		try {
			Globals.setGlobal("foo", "${bar}");
		} catch (RuntimeException ex) {
			assertThat(ex.getMessage(), is("illegal global variable value for foo '${bar}' -- "
				+ Globals.ILLEGAL_VALUE_MSG));
			assertThat(Globals.getGlobals().size(), is(0));
			return;
		}
		fail("should have thrown exception");
	...
	@Test
	public void testSetFromMapWithIllegalVariableValue() {
		assertThat(Globals.getGlobals(), notNullValue());
		assertThat(Globals.getGlobals().size(), is(0));
	...
	@Test
	public void testSetFromFileWithIllegalVariableValue() throws IOException {
		try {
			File dir = FileUtils.tempDir();
			File globals = new File(dir, "globals.properties");
			FileUtils.writeFile(globals, "foo=${bar}");
			Globals.setGlobals(globals);
		} catch (RuntimeException ex) {
			assertThat(
					ex.getMessage(),
					containsString("globals file 'globals.properties' has illegal global variable value for foo '${bar}'"));
			assertThat(Globals.getGlobals().size(), is(0));
			return;
		}
		fail("should have thrown exception");
	...
	@Test
	public void testParseFromStringWithIllegalVariableValue() {
		try {
			Map<String, String> m = Globals.parse("foo=${bar}");
			assertThat(m.size(), is(0));
		} catch (RuntimeException ex) {
			assertThat(ex.getMessage(), is("illegal global variable value for foo '${bar}' -- " + Globals.ILLEGAL_VALUE_MSG));
			assertThat(Globals.getGlobals().size(), is(0));
			return;
		}
		fail("should have thrown exception");
