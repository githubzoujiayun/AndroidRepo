...
public class ArgumentDialogCellEditor extends DialogCellEditor {

	static public interface IViewUpdater {
		void update(Object value);
	}

	Map<String, String> nameValPairs = new HashMap<String, String>();
	TableRow currentRow;
	final IViewUpdater viewUpdater;
	private Text textField;

	protected ArgumentDialogCellEditor(Composite parent, Map<String, String> nameValPairs,
			TableRow currentRow, IViewUpdater viewUpdater) {
		super(parent);
	...
	protected Object openDialogBox(Control cellEditorWindow) {
		ArgumentEditorDialog ftDialog = new ArgumentEditorDialog(cellEditorWindow.getShell(),
				nameValPairs, currentRow);
	...
	protected void setValueToModel() {
		...
		if (newValidState) {
			markDirty();
			doSetValue(newValue);
			currentRow.setArgsAndModifiers(newValue + " " + currentRow.getModifiersAsString());
			if (viewUpdater != null) {
				viewUpdater.update(newValue);
			}
		} else {
