...
+ (void)load {
    if (self == [UINavigationController class]) {
        ...
        Method replacedMethod =
            class_getInstanceMethod(self, @selector(mtpushViewController:animated:));
        method_exchangeImplementations(originalMethod, replacedMethod);

        Method originalMethod2 =
            class_getInstanceMethod(self, @selector(popViewControllerAnimated:));
        Method replacedMethod2 =
            class_getInstanceMethod(self, @selector(mtpopViewControllerAnimated:));
...
- (void)mtpushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    float animationDuration = animated ? 1.0 / 2.0 : 0;
    [MonkeyTalk sharedMonkey].isPushingController = YES;
    
    if ([[MonkeyTalk sharedMonkey].commandSpeed doubleValue] < 333333 &&
        [MonkeyTalk sharedMonkey].commandSpeed)
        animated = NO;

    [self mtpushViewController:viewController animated:animated];
    [self performSelector:@selector(endPushViewController)
               withObject:nil
...
- (UIViewController *)mtpopViewControllerAnimated:(BOOL)animated {
    ...
    if ([MonkeyTalk sharedMonkey].commandSpeed &&
        [[MonkeyTalk sharedMonkey].commandSpeed doubleValue] < 333333)
        animated = NO;
