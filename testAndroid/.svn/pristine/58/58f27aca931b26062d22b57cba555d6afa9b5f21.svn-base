...
public abstract class AutomatorBase implements IAutomator {
	...
	public String getValue(String propertyPath) {
		...
		Object o = getValue2(pathElements[0]);

		for (int i = 1; i < pathElements.length && o != null; i++) {
			o = getValue2(o, pathElements[i]);
		}
	 ...
	 * @return the component's current value for the supplied propertyPath expression.
	 */
	protected Object getValue2(String propertyPath) {
		return getValue2(getComponent(), propertyPath);
	}

	protected Object getValue2(Object obj, String propertyPath) {
		...
		for (int i = 0; i < m.length; i++) {
			...
			if (m[i].getName().equals(getter) && m[i].getParameterTypes().length == 0) {
				try {
					Object result = m[i].invoke(obj, (Object[]) null);
			...
			if (m[i].getName().equals(iser) && m[i].getParameterTypes().length == 0) {
				...
				String result = null;
				try {
					result = ((Boolean) m[i].invoke(obj, (Object[]) null)).toString();
					return result;
				} catch (Exception e) {
	...
	private boolean isValidVerifyAction(String action) {
		return (action.equalsIgnoreCase(AutomatorConstants.ACTION_VERIFY_IMAGE)
				...
				|| action.equalsIgnoreCase(AutomatorConstants.ACTION_VERIFY_WILDCARD) || action
					.equalsIgnoreCase(AutomatorConstants.ACTION_VERIFY_NOT_WILDCARD));
	}
