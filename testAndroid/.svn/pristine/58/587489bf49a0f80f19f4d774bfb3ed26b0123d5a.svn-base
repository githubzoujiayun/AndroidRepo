 ...
 */
public class API {
	private static Map<ScriptType, String[]> scriptTypes = new EnumMap<ScriptType, String[]>(
			ScriptType.class);
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    ...
    new Action(
      ...
      new ArrayList<Arg>(Arrays.asList(
        new Arg("timeout","how long to wait for the app to restart (in ms)","int")
      )),
      "void",
      "")
    ))
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    ...
    new Action(
      ...
      new ArrayList<Arg>(Arrays.asList(
        new Arg("filter","(optional) a wildcard pattern to filter by component type (ex: But* would show only Button and ButtonSelector components in the tree). Defaults to all components.","String"),
        new Arg("format","(optional) the output format (text or json). Defaults to text.","String")
      )),
      "void",
      "")
    ))
...
components.add(new Component(
  ...
  ,
  null,
  EnumSet.noneOf(ScriptType.class)
  )
);
components.add(new Component(
  "Globals",
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    new Action(
      ...
      new ArrayList<Arg>(Arrays.asList(
        new Arg("index","the index of the row to scroll to.","int")
      )),
      "void",
      ""),
    new Action(
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    new Action(
      "Define",
      "Define local variables used in the script with an optional default value. Must be the first command in a script if you want to call the script from another script while passing in values for the local variables.",
      new ArrayList<Arg>(Arrays.asList(
        new Arg("args","the local variables, in the form of: var=default var2=default2","String",true)
      )),
      "void",
      ""),
    new Action(
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    new Action(
      ...
      new ArrayList<Arg>(Arrays.asList(
        new Arg("x","x-coordinate of the tap, in component coords","int"),
        new Arg("y","y-coordinate of the tap, in component coords","int")
      )),
      "void",
      ""),
    new Action(
...
components.add(new Component(
  ...
  new ArrayList<Action>(Arrays.asList(
    new Action(
      ...
      new ArrayList<Arg>(Arrays.asList(
        new Arg("method","the method to call","String"),
        new Arg("args","the args to be supplied to the method","String",true)
      )),
      "void",
      ""),
    new Action(
...
// codegen-end
	...
	public static String[] getComponentNames(ScriptType scriptType) {
		Map<ScriptType, List<String>> scriptTypes = new EnumMap<ScriptType, List<String>>(
				ScriptType.class);

		if (scriptTypes.isEmpty()) {
			for (Component component : components) {
				String componentName = component.getName();
				for (ScriptType type : component.getScriptTypes()) {
					List<String> componentNames = scriptTypes.get(type);
					if (componentNames == null) {
						componentNames = new LinkedList<String>();
						scriptTypes.put(type, componentNames);
					}
					componentNames.add(componentName);
				}
			}
			for (Map.Entry<ScriptType, List<String>> entry : scriptTypes.entrySet()) {
				Collections.sort(entry.getValue());
				String[] value = {};
				value = entry.getValue().toArray(value);
				API.scriptTypes.put(entry.getKey(), value);
			}
		}
		String[] componentNames = API.scriptTypes.get(scriptType);
		return componentNames;
	}

	/**
	 * True if the given component type name is found (case-insensitive) in the list of all
	 * components, otherwise false.
	 * 
	 * @return true if component type exists, otherwise false
	 */
	public static boolean hasComponentType(String componentType) {
	...
	/**
	 * Find (case-insensitive) the MonkeyTalk component from the given component type name. Returns
