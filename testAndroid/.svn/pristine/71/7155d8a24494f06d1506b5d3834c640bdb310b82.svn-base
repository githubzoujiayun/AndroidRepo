...
public class ADBHelper {
	...
	public static String validate() {
		...
		if (path == null) {
			DirectoryDialog dirDialog = new DirectoryDialog(PlatformUI.getWorkbench()
					.getActiveWorkbenchWindow().getShell());
			dirDialog.setText("Select your Android SDK location");
			path = dirDialog.open();

			if (path == null) {
				return "Android SDK path was not set";
			} else {
				FoneMonkeyPlugin.getDefault().getPreferenceStore()
						.setValue(PreferenceConstants.P_ANDROIDHOME, path);
			}
	...
	public static String validateAndroidSdkPath(String path) {
		...
		if (!adb.canExecute()) {
			return "ADB is not executable. Found here: " + adb.getAbsolutePath();
		}
