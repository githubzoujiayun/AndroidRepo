...
aspect OnCreate {
	...
	after(): captureOnCreate()
	{
		...
		View decorView = a.getWindow().peekDecorView();
		if(decorView != null){
			FunctionalityAdder.walkTree(decorView.getRootView());
		}
	}
	...
	{
		try {
			View decorView = ActivityManager.getCurrentDialog().getWindow().getDecorView();
			if (decorView !=null) {
				FunctionalityAdder.walkTree(decorView.getRootView());
			}
		} catch (Exception ex) {
	...
	{
		if (p.getWindow() != null) {
			View v = p.getWindow().peekDecorView();
	...
	after(final Object dialogFragment) returning: captureDialogFragmentShow(dialogFragment)
	{
		Class<?> clazz;
			...
			public void run() {
				...
				try {
					...
					if (dialog.getWindow() != null) {
						View v = dialog.getWindow().peekDecorView();
						FunctionalityAdder.walkTree(v);
					}
				} catch (Exception e) {
	...
	{
		ActivityManager.setCurrentDialog(d);
		View v = d.getWindow().peekDecorView();
		FunctionalityAdder.walkTree(v);
	}
}