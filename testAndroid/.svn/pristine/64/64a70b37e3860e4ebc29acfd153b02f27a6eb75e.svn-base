            ...
            if ([gr isKindOfClass:[UILongPressGestureRecognizer class]]) {
                ...
                for (NSNumber *state in playbackStates) {
                    ...
                    for (int i = 0; i < [grProxy->_targets count]; i++) {
                        ...
                        if (selector != @selector(loupeGesture:)) {
                            [target performSelectorOnMainThread:selector withObject:longPress waitUntilDone:YES];
                        }
                    }
                }
            }
...
- (void) mtreplaceAction:(UIGestureRecognizer *)recognizer {
    ...
    if (isWebTouch || isTapInWebView || isLongPressInWebView) {
        ...
        if (isWebTouch) {
            ...
            NSString *type = [webRecognizer respondsToSelector:@selector(_typeDescription)] ? [NSString stringWithFormat:@"%@",webRecognizer._typeDescription] : nil;
            
            if (!type) {
                type = webRecognizer.type == 2 ? @"WebEventTouchEnd" : @"WebEventTouchOther";
            }
