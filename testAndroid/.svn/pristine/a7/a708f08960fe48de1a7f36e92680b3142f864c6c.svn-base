...
public class ExecTest {
	...
	@Test
	public void testExecWithUTF8() throws IOException {
		ExecResult result = Exec.run("/bin/echo Héìíô");
		assertThat(result.getStatus(), is(ExecStatus.OK));
		assertThat(result.getMessage(), is("Héìíô"));
		assertThat(result.getStdout(), is("Héìíô"));
		assertThat(result.getStderr(), is(""));
		assertThat(result.getExitValue(), is(0));
		assertThat(result.timedOut(), is(false));
		assertThat(
				result.toString(),
				is("ExecResult (status=OK message=Héìíô exitValue=0 stdout=(5 chars) stderr=(0 chars) timedOut=false)"));
	}
}