...
- (NSString *)executeMonkeyTalkJs:(NSString *)js {
    std::string compiled("");
    for (size_t i = 0; MTwebdriver::MTatoms::EXECUTE_SCRIPT[i] != NULL; i++) {
        compiled.append(MTwebdriver::MTatoms::EXECUTE_SCRIPT[i]);
    }
    ...
    NSString* result = [webViewController jsEval:@"(%@)(%@,%@,true)",
                        [NSString stringWithCString:compiled.c_str() encoding:NSUTF8StringEncoding],
                        [NSString stringWithFormat:@"function(){%@}",js],
                        @"null"];
    NSDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:[result dataUsingEncoding:NSUTF8StringEncoding] options:0 error:nil];
    return [jsonDict objectForKey:@"value"];
}
