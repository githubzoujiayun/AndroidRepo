...
@interface MTDDTTYLoggerColorProfile : NSObject {
@public
	int mask;
	int context;
...
static MTDDTTYLogger *sharedInstance;
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		sharedInstance = [[MTDDTTYLogger alloc] init];
	}
}

+ (MTDDTTYLogger *)sharedInstance
{
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		MTDDTTYLoggerColorProfile *newColorProfile =
		    [[MTDDTTYLoggerColorProfile alloc] initWithForegroundColor:txtColor
		...
		NSUInteger i = 0;
		for (MTDDTTYLoggerColorProfile *colorProfile in colorProfilesArray)
		{
...
- (void)logMessage:(MTDDLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		MTDDTTYLoggerColorProfile *colorProfile = nil;
		...
		if (colorsEnabled)
		{
			...
			if (colorProfile == nil)
			{
				for (MTDDTTYLoggerColorProfile *cp in colorProfilesArray)
				{
...
@implementation MTDDTTYLoggerColorProfile
...
- (id)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(int)aMask context:(int)ctxt
{
	if ((self = [super init]))
	{
		...
		if (fgColor)
		{
			[MTDDTTYLogger getRed:&r green:&g blue:&b fromColor:fgColor];
		...
		if (bgColor)
		{
			[MTDDTTYLogger getRed:&r green:&g blue:&b fromColor:bgColor];
		...
		if (fgColor && isaColorTTY)
		{
			...
			fgCodeIndex = [MTDDTTYLogger codeIndexForColor:fgColor];
		...
		if (bgColor && isaColorTTY)
		{
			...
			bgCodeIndex = [MTDDTTYLogger codeIndexForColor:bgColor];
