...
public class AdapterViewAutomator extends ViewAutomator implements OnItemClickListener,
	...
	/**
	 * 
	 * search for text in the view and child views
	 * @author chao.qin
	 * @date 2015-07-13
	 * @return text ,else return null
	 */
	private String findText(View view) {
		...
		if (view instanceof TextView) {
			...
			for(int i=0;i<count;i++) {
				String text = findText(parent.getChildAt(i));
	...
	private void recordSelection(int position, boolean isLong) {
		...
		// get the texts in the view by the target position,
		// because uiautomator cannot get view from a adapter view
		// by chao.qin / 2015-07-13
		if (adapterView instanceof AbsListView) {
			AbsListView absAdapterView = (AbsListView) adapterView;
			View view = null;
			try {
				Method m = AbsListView.class.getDeclaredMethod("obtainView", int.class,
						boolean[].class);
				m.setAccessible(true);
				view = (View) m.invoke(absAdapterView, position,
						new boolean[] { false });
		...
		if (obj instanceof String) {
			...
			record(isLong ? AutomatorConstants.ACTION_LONG_SELECT_INDEX
					: AutomatorConstants.ACTION_SELECT_INDEX,
