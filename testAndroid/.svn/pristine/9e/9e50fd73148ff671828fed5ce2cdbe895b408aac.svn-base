...
public class DetailReportHtml {
	...
	protected static final int SPAN_MAX = 24;

	/** Max offset allowed (so we don't indent too far) */
	protected static final int OFFSET_MAX = 15;
	...
	public String createDetailReportHtml(PlaybackResult result, String xmlReport) throws Exception {
		rowTmpl = FileUtils.readStream(DetailReportHtml.class.getResourceAsStream(ROW));
		extraTmpl = FileUtils.readStream(DetailReportHtml.class.getResourceAsStream(EXTRA));
		screenshotsTmpl = FileUtils.readStream(DetailReportHtml.class
				.getResourceAsStream(SCREENSHOTS));
		metricsTmpl = FileUtils.readStream(DetailReportHtml.class.getResourceAsStream(METRICS));

		String html = FileUtils.readStream(DetailReportHtml.class.getResourceAsStream(DETAIL));
		...
		html = html.replace("${metrics_chart}", getMetricsChart(detail));
		return html;
	}

	protected String getMetricsChart(Element eleml) throws Exception {
		return "";
	}
	 ...
	 */
	protected String getContent(Element elem, int level) throws Exception {
	 ...
	 * @throws IOException
	 */
	protected String getHtmlRow(Element elem, int offset) throws IOException {
		...
		contents = contents.replace("{expandId}", "expand" + idx);

		// screenshots & metrics
		String extra = getExtra(elem, offset + 1);
		if (extra.length() > 0) {
			contents = contents.replace("${onclick}", " onclick=\"toggle_extra('" + idx + "');\"");
			contents = contents.replace("{expandClass}", "pbox-expand");
			contents += extra;
		} else {
			contents = contents.replace("${onclick}", "");
			contents = contents.replace("{expandClass}", "pbox-no-expand");
		}
	...
	protected String getRightSide(Element elem) {
		String right = "";
	 ...
	 * @throws IOException
	 */
	protected String getExtra(Element elem, int offset) {
		if (elem == null) {
			return "";
		}
	...
	protected String getMetrics(Element elem, int offset) {
		if (elem == null) {
			return "";
		}
	...
	protected String getScreenshots(Element elem, int offset) {
		if (elem == null) {
			return "";
		}
		if (elem.attributeValue("beforeScreenshot") == null
	 ...
	 */
	protected String getRowClass(Element elem) {
	...
	public String getRowTmpl() {
		return rowTmpl;
	}

	public String getExtraTmpl() {
		return extraTmpl;
	}

	public String getScreenshotsTmpl() {
		return screenshotsTmpl;
	}

	public String getMetricsTmpl() {
		return metricsTmpl;
	}
}
