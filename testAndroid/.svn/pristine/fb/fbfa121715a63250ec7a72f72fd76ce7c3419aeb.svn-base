	...
	public PlaybackResult doVerifyImage(File expectedFile, boolean shouldCreateExpectedFile,
		...
		try {
			if (this.screenshot != null) {
				image = FileUtils.readFileToByteArray(screenshot);
			} else {
				...
				image = getImageForVerify(result);
			...
			if (dimensions.length == 4) {
				try {
					...
					if (x < 0 || y < 0 || w < 0 || h < 0) { // make sure dimensions are valid
						result.setStatus(PlaybackStatus.ERROR);
						result.setMessage("\"" + cmd.getComponentType() + "\" with MonkeyID, \""
								+ cmd.getMonkeyId() + "\" has invalid dimensions!");
						return null;
					}
