...
public class Globals {
  ...
  public static final String INVALID_GLOBAL_VALUE_PATTERN = "\\$\\{.*\\}";
  public static final String ILLEGAL_VALUE_MSG = "global variable values may not contain a variable reference, ${<variable>}";
	...
	public static void setGlobals(File f) throws IOException {
        ...
        validateValue(key, val, "globals file '" + f.getName() + "' has");
	 ...
	 * variable name or value in the given string is illegal. Used to parse the {@code globals} 
	 * attribute in the {@code monkeytalk-ant} project.
	 * 
	 * @param s
		...
		if (s != null && s.length() > 0) {
			...
			while (m.find()) {
				...
				if (parts.length > 1) {
					...
					validateValue(parts[0], val, null);
					map.put(parts[0], val);
				}
			}
		}
	...
	public static void validateName(String name, String msg) throws RuntimeException {
	 ...
	 /**
   * Validate the given global variable value. Throws an exception if value is illegal, otherwise
   * does nothing. The given extra message is prepended to the exception message. Global variable 
   * value validation is done automatically by {@link Globals#setGlobal(String, String)}.
   * 
   * @param name
   *            the variable name
   * @param value
   ...
   *             if value is illegal.
   */
  public static void validateValue(String name, String value, String msg) throws RuntimeException {
    if (value.matches(Globals.INVALID_GLOBAL_VALUE_PATTERN)) {
      throw new RuntimeException((msg != null ? msg + " " : "") + "illegal global variable value for " + name + " '" 
         + value + "' -- " + ILLEGAL_VALUE_MSG);
    }
  }
