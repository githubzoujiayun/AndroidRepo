...
public class Command implements Cloneable {
	...
	/**
	 * Set the MonkeyTalk command args from the given list
	 * 
	 * @param args
	 *            a List<String> of args. Can be empty, not null
	 */
	public void setArgs(List<String> args) {
		if (args != null) {
			resetProperties(getComponentType(), getMonkeyId(), getAction(), args, getModifiers());
			setCommand();
		}
	}
	...
	public int getRetryDelay() {
		return getIntModiferValueWithDefault(RETRY_DELAY_MODIFIER, DEFAULT_RETRYDELAY);
	}
	...
	/**
	 * get the screenshot on error value
	 * 
	 * @return the screenshot on error value, if it has been explicitly set, else null
	 */
	public Boolean getScreenshotOnError() {
		String modifierValue = modifiers.get(SCREENSHOT_ON_ERROR);
		if (modifierValue != null) {
			return isScreenshotOnError();
		} else if (command.toLowerCase().contains(MODIFIER_PREFIX + SCREENSHOT_ON_ERROR)) {
			return true;
		}
		return null;
	}
	...
	private void resetProperties(String componentType, String monkeyId, String action,
	...
	public JSONObject getCommandAsJSON(boolean withTimings) {
		...
		try {
			...
			if (withTimings) {
				mods.put(TIMEOUT_MODIFIER, getTimeout());
				mods.put(THINKTIME_MODIFIER, getThinktime());
				if (!isScreenshotOnError()) {
					mods.put(SCREENSHOT_ON_ERROR, "false");
				}
			}
