...
// We probably shouldn't be using MTDDLog() statements within the MTDDLog implementation.
...
// The "global logging queue" refers to [MTDDLog loggingQueue].
...
@interface MTDDLoggerNode : NSObject {
@public 
	id <MTDDLogger> logger;	
	dispatch_queue_t loggerQueue;
}

+ (MTDDLoggerNode *)nodeWithLogger:(id <MTDDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue;

@end


@interface MTDDLog (PrivateAPI)

+ (void)lt_addLogger:(id <MTDDLogger>)logger;
+ (void)lt_removeLogger:(id <MTDDLogger>)logger;
+ (void)lt_removeAllLoggers;
+ (void)lt_log:(MTDDLogMessage *)logMessage;
+ (void)lt_flush;
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		NSLogDebug(@"MTDDLog: Using grand central dispatch");
		...
		NSLogDebug(@"MTDDLog: numProcessors = %u", numProcessors);
...
+ (void)queueLogMessage:(MTDDLogMessage *)logMessage asynchronously:(BOOL)asyncFlag
{
     ...
     format:(NSString *)format, ...
{
	...
	if (format)
	{
		...
		MTDDLogMessage *logMessage = [[MTDDLogMessage alloc] initWithLogMsg:logMsg
		                                                          level:level
		                                                           flag:flag
		                                                        context:context
		                                                           file:file
...
+ (BOOL)isRegisteredClass:(Class)class
{
	...
	// 
	// The method +[MTDDLog isRegisteredClass] will crash a project when using it with ARC + Simulator.
...
+ (NSArray *)registeredClasses
{
	...
	// We can now loop through the classes, and test each one to see if it is a MTDDLogging class.
...
+ (void)lt_addLogger:(id <MTDDLogger>)logger
{
	...
	MTDDLoggerNode *loggerNode = [MTDDLoggerNode nodeWithLogger:logger loggerQueue:loggerQueue];
...
+ (void)lt_removeLogger:(id <MTDDLogger>)logger
{
	...
	MTDDLoggerNode *loggerNode = nil;
	
	for (MTDDLoggerNode *node in loggers)
	{
	...
	if (loggerNode == nil)
	{
		NSLogDebug(@"MTDDLog: Request to remove logger which wasn't added");
		return;
	}
...
+ (void)lt_removeAllLoggers
{
	...
	for (MTDDLoggerNode *loggerNode in loggers)
	{
...
@implementation MTDDLoggerNode

- (id)initWithLogger:(id <MTDDLogger>)aLogger loggerQueue:(dispatch_queue_t)aLoggerQueue
{
	if ((self = [super init]))
	{
		logger = aLogger;
...
+ (MTDDLoggerNode *)nodeWithLogger:(id <MTDDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue
{
	return [[MTDDLoggerNode alloc] initWithLogger:logger loggerQueue:loggerQueue];
}
             ...
             options:(MTDDLogMessageOptions)optionsMask
{
	if ((self = [super init]))
	{
		...
		if (options & MTDDLogMessageCopyFile)
			file = dd_str_copy(aFile);
		else
			file = (char *)aFile;
		
		if (options & MTDDLogMessageCopyFunction)
...
- (NSString *)fileName
{
	return MTDDExtractFileNameWithoutExtension(file, NO);
}
...
- (void)dealloc
{
	if (file && (options & MTDDLogMessageCopyFile))
		free(file);
	
	if (function && (options & MTDDLogMessageCopyFunction))
		free(function);
...
@implementation MTDDAbstractLogger
...
- (id <MTDDLogFormatter>)logFormatter
{
	...
	//
	// MTDDLogVerbose(@"log msg 1");
	// MTDDLogVerbose(@"log msg 2");
	// [logger setFormatter:myFormatter];
	// MTDDLogVerbose(@"log msg 3");
	//
	...
	// IMPORTANT NOTE:
	// 
	// Methods within the MTDDLogger implementation MUST access the formatter ivar directly.
	...
	dispatch_queue_t globalLoggingQueue = [MTDDLog loggingQueue];
	
	__block id <MTDDLogFormatter> result;
...
- (void)setLogFormatter:(id <MTDDLogFormatter>)logFormatter
{
