 ...
 * 
 * Step 3:
 * Replace your NSLog statements with MTDDLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> MTDDLogError(@"Fatal error, no dohickey found!");
 * 
 * MTDDLog works exactly the same as NSLog.
 * This means you can pass it multiple variables just like NSLog.
**/


@class MTDDLogMessage;

@protocol MTDDLogger;
@protocol MTDDLogFormatter;
 ...
 * 
 * The MTDDLogMessage object includes a 'tag' ivar that may be used for a variety of purposes.
 ...
 * as the MTDDLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         MTDDLogVerbose(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define MTDDLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define MTDDLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define MTDDLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define MTDDLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

#define MTDDLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   ddLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define MTDDLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    ddLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define MTDDLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    ddLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define MTDDLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, ddLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: MTDDLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/

NSString *MTDDExtractFileNameWithoutExtension(const char *filePath, BOOL copy);

#define THIS_FILE (MTDDExtractFileNameWithoutExtension(__FILE__, NO))

/**
 * The THIS_METHOD macro gives you the name of the current objective-c method.
 * 
 * For example: MTDDLogWarn(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
...
+ (void)addLogger:(id <MTDDLogger>)logger;
+ (void)removeLogger:(id <MTDDLogger>)logger;
...
@protocol MTDDLogger <NSObject>
@required

- (void)logMessage:(MTDDLogMessage *)logMessage;
...
**/
- (id <MTDDLogFormatter>)logFormatter;
- (void)setLogFormatter:(id <MTDDLogFormatter>)formatter;
 ...
 * 
 * This allows invocations of MTDDLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that MTDDLog's flushLog method is invoked automatically when the application quits,
...
@protocol MTDDLogFormatter <NSObject>
@required
...
**/
- (NSString *)formatLogMessage:(MTDDLogMessage *)logMessage;
...
**/
- (void)didAddToLogger:(id <MTDDLogger>)logger;
- (void)willRemoveFromLogger:(id <MTDDLogger>)logger;
...
/**
 * The MTDDLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
**/

enum {
	MTDDLogMessageCopyFile     = 1 << 0,
	MTDDLogMessageCopyFunction = 1 << 1
};
typedef int MTDDLogMessageOptions;

@interface MTDDLogMessage : NSObject
{
...
@public
	...
	// For 3rd party extensions that manually create MTDDLogMessage instances.
	MTDDLogMessageOptions options;
}
 ...
 * Options is a bitmask which supports MTDDLogMessageCopyFile and MTDDLogMessageCopyFunction.
**/
- (id)initWithLogMsg:(NSString *)logMsg
            ...
            function:(const char *)function
                line:(int)line
                 tag:(id)tag
             options:(MTDDLogMessageOptions)optionsMask;
...
/**
 * The MTDDLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface MTDDAbstractLogger : NSObject <MTDDLogger>
{
	id <MTDDLogFormatter> formatter;
