...
OnLongClickListener, OnCreateContextMenuListener {
	...
	public List<String> getIdentifyingValues() {
		...
		list.addAll(getRawMonkeyIdCandidates());
		return list;
	}
	...
	public int getOrdinal() {
		if (!ordInit || ordinal == -1) {
			ordinal = findOrdinalFor(getView());
			ordInit = true;
		}
		return ordinal;
	}
	...
	public static int findOrdinalFor(View target, Class<?> targetComponentType) {
		...
		Set<View> roots = new HashSet<View>(AutomationManager.getRoots());
		for (View r : roots) {
	...
	public String play(final String action, final String... args) {
		...
		if (action.equalsIgnoreCase(AutomatorConstants.ACTION_FIND)) {
			AlphaAnimation a = new AlphaAnimation((float) 1.0, (float) 0.2);
			a.setDuration(300);
			a.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					AlphaAnimation a = new AlphaAnimation((float) 0.2, (float) 1.0);
					a.setDuration(300);
					ViewAutomator.this.getView().setAnimation(a);
				}
			});
			ViewAutomator.this.getView().setAnimation(a);
	...
	private void swipe(String direction) {
		...
		if (isVertical) {
			startX = getView().getWidth() / 2;
			endX = startX;
			if (isUp) {
				startY = getView().getHeight() - 1;
				endY = 1;
			} else {
				startY = 1;
				endY = getView().getHeight() - 1;
			}
		} else {
			startY = getView().getHeight() / 2;
			endY = startY;
			if (direction.equalsIgnoreCase("left")) {
				startX = getView().getWidth() - 1;
				endX = 1;
			} else {
				startX = 1;
				endX = getView().getWidth() - 1;
				;
	...
	protected void scroll(final int x, final int y) {
		AutomationManager.runOnUIThread(new Runnable() {
			@Override
			public void run() {
				View view = getView();
	...
	public Map<Integer, String> getFieldMap() {
		if (idMap == null) {
			idMap = new HashMap<String, Map<Integer, String>>();
		}
		...
		Map<Integer, String> packageIdMap = new HashMap<Integer, String>();

		Class<?> r;
		String rClass = packageName + ".R$id";
		try {
		...
		for (Field f : r.getFields()) {
			...
			packageIdMap.put(val, f.getName());

		}
		// cache at end
		idMap.put(packageName, packageIdMap);
		return packageIdMap;
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		ActivityManager.setCurrentMenu(Window.FEATURE_CONTEXT_MENU, menu);
	}
