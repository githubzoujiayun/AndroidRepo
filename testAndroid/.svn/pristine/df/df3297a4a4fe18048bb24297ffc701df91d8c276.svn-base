...
- (void) mtAssureAutomationInit {
	...
	if (!self.delegate) {
        MTDefaultWebViewDelegate *del = [[[MTDefaultWebViewDelegate alloc] init] retain];
		self.delegate = del;
	}
}
...
- (void) mtSetDelegate:(NSObject <UIWebViewDelegate>*) del {
    ...
    if ([self.delegate class] != [MTWebViewController class]) {
        MTWebViewController *webController = [[[MTWebViewController alloc] init] retain];
...
- (void) playbackMonkeyEvent:(id)event {
    ...
    } else if([commandEvent.command isEqualToString:MTCommandClearCookies ignoreCase:YES]){
        NSHTTPCookie *cookie;
...
- (void) recordTap:(UIGestureRecognizer *)tapGesture {
    ...
    if (json) {
        ...
        NSArray *args = [json objectForKey:@"args"] && [[json objectForKey:@"args"] length] > 0 ? [NSArray arrayWithObject:[json objectForKey:@"args"]] : nil;
        MTCommandEvent *event = [MTCommandEvent command:command className:component monkeyID:monkeyId args:args];
