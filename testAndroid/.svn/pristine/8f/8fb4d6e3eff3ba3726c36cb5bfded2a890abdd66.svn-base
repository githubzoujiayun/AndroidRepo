if (MonkeyTalk == undefined || adapters == undefined) {
	var MonkeyTalk = {};
    MonkeyTalk.shouldRecordScroll = false;
    ...
    MonkeyTalk.getElementValuesFromXpath = function (expression) {
        //wgxpath.install();
        var xpath = document.evaluate(expression, document, null, XPathResult.ANY_TYPE, null);
        if (xpath != undefined && xpath != null) {
            ...
            while (element != null) {
                ...
                var v = adapter.defaultValue(element);
                
                if (v != undefined && v != null) {
                    elementValues[i] = adapter.defaultValue(element);
                    i++;
                }
	...
	MonkeyTalk.getComponentTreeEntry = function(element) {
        ...
        // this line gets preserved in MTWebRecorder.m
        var match = /\r|\n/.exec(comp["monkeyId"]);
            ...
            var transform = element.style.webkitTransform;
 			...
 			if (element.scrollTop > 0) {
                ...
                if (transform.indexOf("translate") > -1) {
                    var matrix = new WebKitCSSMatrix(element.style.webkitTransform);
    ...
    // default value
    adapters.tag.prototype.defaultValue = function(e) {
    ...
    adapters.tag.prototype.rawOrdinal = function(e) {
        // Find all elements in the DOM
        var elements = document.getElementsByTagName('*');
        var ordinal = 0;
    ...
    adapters.tag.prototype.getMTCommand = function(element, action) {
        ...
        if (shouldFindOrdinal) {
            componentType = "View";
            monkeyId = "xpath=//" + element.tagName.toLowerCase() + "[" + this.rawOrdinal(element) + "]";
        }
    ...
    // default value
    adapters.input.prototype.defaultValue = function(e) {
        var isRadio = e.type.toLowerCase() == 'radio';
        var isCheckbox = e.type.toLowerCase() == 'checkbox';
        if (isRadio && !e.checked) {
            return null;
        }
    ...
    // default value
    adapters.select.prototype.defaultValue = function(e) {
        return e.value;
    };
    ...
    document.addEventListener('touchmove', function(e){
                              MonkeyTalk.shouldRecordScroll = true;
                              }, false);
    
    document.addEventListener('touchend', function(e){
                              if (MonkeyTalk.shouldRecordScroll) {
