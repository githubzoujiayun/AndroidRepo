...
@interface MTWebSocket (PrivateAPI)
...
+ (BOOL)isWebSocketRequest:(MTHTTPMessage *)request
{
	// Request (Draft 75):
	// 
	// GET /demo HTTP/1.1
	...
	else if (![upgradeHeaderValue caseInsensitiveCompare:@"MTWebSocket"] == NSOrderedSame) {
		isWebSocket = NO;
	}
...
+ (BOOL)isVersion76Request:(MTHTTPMessage *)request
{
...
+ (BOOL)isRFC6455Request:(MTHTTPMessage *)request
{
...
- (id)initWithRequest:(MTHTTPMessage *)aRequest socket:(MTGCDAsyncSocket *)socket
{
	HTTPLogTrace();
...
- (void)sendResponseHeaders
{
	...
	MTHTTPMessage *wsResponse = [[MTHTTPMessage alloc] initResponseWithStatusCode:101
	                                                              description:@"Web Socket Protocol Handshake"
	                                                                  version:HTTPVersion1_1];
	
	[wsResponse setHeaderField:@"Upgrade" value:@"MTWebSocket"];
...
- (void)socketDidDisconnect:(MTGCDAsyncSocket *)sock withError:(NSError *)error
{
