...
public class CommandSender extends Sender {
	...
	/**
	 * Send the dump tree command to the agent, with the specified timeout. Returns the UI component tree of the app (native
	 * components, plus web components).
	 */
	public Response dumpTree(Integer timeout) {
		JSONObject json = new JSONObject();
		if (timeout==null || timeout<0) {
			timeout = DEFAULT_TREE_TIMEOUT;
		}
		JSONObject modifiers = new JSONObject();
		try {
			modifiers.put("timeout", timeout);
			json.put("modifiers", modifiers);
		} catch (JSONException ex) {
			json = new JSONObject();
		}
		return sendCommand(DUMPTREE, json);
	}
