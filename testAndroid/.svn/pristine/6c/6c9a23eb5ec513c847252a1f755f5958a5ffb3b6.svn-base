...
+ (void)load {
    if (self == [UITextField class]) {
        ...
        Method originalDelMethod = class_getInstanceMethod(self, @selector(delegate));
        Method replacedDelMethod = class_getInstanceMethod(self, @selector(mtDelegate));
        method_exchangeImplementations(originalDelMethod, replacedDelMethod);
    }
}
...
- (id<UITextFieldDelegate>)mtDelegate {
    id del = [self mtDelegate];
    
    if (!del) {
        del = [[MTDefaultTextFieldDelegate alloc] init];
    }
    return del;
}
...
- (void) playbackMonkeyEvent:(MTCommandEvent*)recevent {
	...
	if ([recevent.command isEqualToString:MTCommandReturn ignoreCase:YES] || 
        ...
        BOOL respondsToChangeChars = [self.delegate respondsToSelector:@selector(textField:shouldChangeCharactersInRange:replacementString:)];
		if (noInput || !respondsToChangeChars || [self.delegate textField:self shouldChangeCharactersInRange:range replacementString:newText]) {
			if (!noInput) {
				self.text = newText;
