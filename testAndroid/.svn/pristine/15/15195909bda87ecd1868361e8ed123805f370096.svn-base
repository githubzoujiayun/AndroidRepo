...
andMTWebViewController:(MTWebViewController *)webVc
    ...
    [self setMyWebDriverHandlerWithGETAction:NULL POSTAction:@selector(click:) withName:@"click"];
    //  [self setMyWebDriverHandlerWithGETAction:NULL
    //                                POSTAction:@selector(clickSimulate:)
    //                                  withName:@"click"];
    
    [self setMyWebDriverHandlerWithGETAction:NULL POSTAction:@selector(clear:) withName:@"clear"];
    
    [self setMyWebDriverHandlerWithGETAction:NULL POSTAction:@selector(submit:) withName:@"submit"];
    
    [self setMyWebDriverHandlerWithGETAction:@selector(text) POSTAction:NULL withName:@"text"];
    ...
    [self setMyWebDriverHandlerWithGETAction:@selector(name) POSTAction:NULL withName:@"name"];
    
    [self setResource:[MTAttribute attributeDirectoryForElement:self] withName:@"attribute"];
    
    [self setResource:[MTCss cssDirectoryForElement:self] withName:@"css"];
    
    [self setResource:[MTElementComparatorBridge comparatorBridgeFor:self] withName:@"equals"];
...
+ (MTElement *)elementWithId:(NSString *)elementId
    ...
    return [[MTElement alloc] initWithId:elementId
                              andSession:session
                  andMTWebViewController:webVc
                            andCommandId:commandId];
}

- (NSDictionary *)idDictionary {
    return [NSDictionary dictionaryWithObjectsAndKeys:[self elementId], kElementIdKey, nil];
}

#pragma mark Webdriver methods

- (NSDictionary *)findElement:(NSDictionary *)query {
    return [self findElement:query root:[self idDictionary] implicitlyWait:[session_ implicitWait]];
}

- (NSArray *)findElements:(NSDictionary *)query {
    return
    [self findElements:query root:[self idDictionary] implicitlyWait:[session_ implicitWait]];
}
...
- (CGPoint)location {
    NSDictionary *result =
    (NSDictionary *)[self executeAtom:MTwebdriver::MTatoms::GET_LOCATION
                             withArgs:[NSArray arrayWithObject:[self idDictionary]]];
    
    NSString *iframejs = @"function(){"
    "var arrFrames = window.parent.document.getElementsByTagName(\"IFRAME\");"
    "var frame = null;"
    "for (var i = 0; i < arrFrames.length; i++) {"
    "if (arrFrames[i].contentWindow === window) frame = arrFrames[i];"
    "}"
    "if (frame == null) return {x:0,y:0};"
    "return {x:frame.offsetLeft,y:frame.offsetTop};"
    "}";
    
    NSDictionary *iframeResult = (NSDictionary *)
    [self executeJsFunction:iframejs withArgs:[NSArray arrayWithObject:[self idDictionary]]];
    
    // position of the element
    NSNumber *x = [result objectForKey:@"x"];
    NSNumber *y = [result objectForKey:@"y"];
    
    // if the element is in the iframe, get the iframe offset and adjust actual x,y
    NSNumber *iX = iframeResult ? [iframeResult objectForKey:@"x"] : [NSNumber numberWithFloat:0.0];
    NSNumber *iY = iframeResult ? [iframeResult objectForKey:@"y"] : [NSNumber numberWithFloat:0.0];
    
    // actual position of element
    float actualX = [x floatValue] + [iX floatValue];
    float actualY = [y floatValue] + [iY floatValue];
    
    // adjust y value if iOS 7 and in nav controller
    if ([MTUtils isOs7Up]) {
        UINavigationController *nextResponder =
        (UINavigationController *)self.webView.nextResponder;
        
        while (nextResponder) {
            if ([nextResponder isKindOfClass:[UINavigationController class]]) {
                if (nextResponder.navigationBarHidden) {
                    actualY += 20;
                } else {
                    actualY += nextResponder.navigationBar.frame.size.height +
                    nextResponder.navigationBar.frame.origin.y;
                }
            }
            nextResponder = (UINavigationController *)nextResponder.nextResponder;
        }
    }
...
- (void)scrollElement:(NSArray *)args {
    NSNumberFormatter * f = [[NSNumberFormatter alloc] init];
    [f setNumberStyle:NSNumberFormatterNoStyle];
    NSString *left = [args objectAtIndex:0];
    NSString *top = [args objectAtIndex:1];
    NSNumber *x = [f numberFromString:left];
    NSNumber *y = [f numberFromString:top];
    NSString *js = [NSString
                    stringWithFormat:@"function(){arguments[0].scrollLeft=%@;arguments[0].scrollTop=%@;if (arguments[0].style.webkitTransform != undefined) {arguments[0].style.webkitTransform = \"translate(%ipx, %ipx) translateZ(0px)\";}}", left, top, -x.intValue, -y.intValue];
    [self executeJsFunction:js withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

// Fetches the size of the element in page coordinates.
- (CGSize)size {
    NSDictionary *result = (NSDictionary *)
    [self executeJsFunction:@"function(){return {width:arguments[0].offsetWidth,"
...
- (NSString *)text {
    return (NSString *)[self executeAtom:MTwebdriver::MTatoms::GET_TEXT
                                withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}
...
- (void)sendKeys:(NSDictionary *)dict {
    ...
    [self executeAtom:MTwebdriver::MTatoms::TYPE
             withArgs:[NSArray arrayWithObjects:[self idDictionary], stringToType, nil]];
...
- (void)type:(NSDictionary *)dict {
    ...
    [self executeJsFunction:@"function(){arguments[0].onchange();}"
                   withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

- (void)setValue:(NSString *)val {
    [self executeJsFunction:[NSString stringWithFormat:@"function(){arguments[0].value = %@}", val]
                   withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

- (NSString *)value {
    return (NSString *)[self executeJsFunction:@"function(){return arguments[0].value;}"
                                      withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

// This method is only valid on option elements, checkboxes and radio buttons.
- (NSNumber *)isChecked {
    return (NSNumber *)[self executeAtom:MTwebdriver::MTatoms::IS_SELECTED
                                withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}
...
- (void)setChecked:(NSDictionary *)ignored {
    [self executeAtom:MTwebdriver::MTatoms::SET_SELECTED
             withArgs:[NSArray arrayWithObjects:[self idDictionary], [NSNumber numberWithBool:YES],
                       nil]];
}
...
- (NSNumber *)isEnabled {
    return (NSNumber *)[self executeAtom:MTwebdriver::MTatoms::IS_ENABLED
                                withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

- (NSNumber *)isDisplayed {
    return (NSNumber *)[self executeAtom:MTwebdriver::MTatoms::IS_DISPLAYED
                                withArgs:[NSArray arrayWithObject:[self idDictionary]]];
}

- (NSDictionary *)locationAsDictionary {
    CGPoint location = [self location];
    return [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:location.x], @"x",
...
- (NSDictionary *)sizeAsDictionary {
    ...
    return [NSDictionary
            dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:size.width], @"width",
...
- (id<MTHTTPResource>)elementWithQuery:(NSString *)query {
    if ([query length] > 0) {
        ...
        if (resource == nil) {
            ...
            NSDictionary *idDict = [NSDictionary dictionaryWithObject:otherId forKey:kElementIdKey];
            resource = [[MTElementComparator alloc] initFor:self compareWith:idDict];
        }
    }
...
- (id)initFor:(MTElementComparatorBridge *)parentDirectory
  compareWith:(NSDictionary *)otherElementId {
    ...
    [parentDirectory_ setResource:[MTWebDriverResource resourceWithTarget:self
...
- (id)compareElements {
    ...
    @try {
        NSArray *args = [NSArray
                         arrayWithObjects:[[parentDirectory_ element] idDictionary], otherElementId_, nil];
        result = [self executeJsFunction:@"function(){return arguments[0]==arguments[1];}"
                                withArgs:args];
    }
    @finally {
        // This is a one shot directory; remove ourselves from the parent directory.
        NSLog(@"Removing comparator tmp directory");
        [parentDirectory_ removeResourceWithName:[otherElementId_ objectForKey:kElementIdKey]];
    }
    return result;
}
