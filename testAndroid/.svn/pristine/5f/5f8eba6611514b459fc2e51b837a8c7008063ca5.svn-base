...
- (void)setupServer {
    if (!httpServer) {
        [MTDDLog addLogger:[MTDDTTYLogger sharedInstance]];
        
        // Create server using our custom MyHTTPServer class
        httpServer = [[GLHTTPServer alloc] init];
        ...
        [httpServer setPort:16863];
        
        // Serve files from our embedded Web folder
        NSString *webPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"Web"];
        //MTDDLogInfo(@"Setting document root: %@", webPath);
...
- (void)startServer {
    ...
    if ([httpServer start:&error]) {
        NSLog(@"Started HTTP Server: http://%@:%hu", httpServer.address, [httpServer listeningPort]);
        //MTDDLogInfo(@"Started HTTP Server: http://%@:%hu", httpServer.address, [httpServer listeningPort]);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //            }
            //
            //            self.viewController.addressLabel.text = httpServer.address;
        });
    } else {
        NSLog(@"Error starting HTTP Server: %@", error);
        //MTDDLogError(@"Error starting HTTP Server: %@", error);
    }
}
...
- (void) playbackMonkeyEvent:(MTCommandEvent*)command {
    ...
    BOOL isVerify = ([command.command isEqualToString:MTCommandVerify ignoreCase:YES] ||
                     [command.command isEqualToString:MTCommandVerifyRegex ignoreCase:YES] ||
                     [command.command isEqualToString:MTCommandVerifyWildcard ignoreCase:YES]);
    ...
    if([source isKindOfClass:objc_getClass("MTFoundElement")]) {
        ...
        UIWebView *webView = [MTOrdinalView currentWebView];
        
        if (isVerify && webView) {
            [MTVerifyCommand handleVerify:command];
        } else {
