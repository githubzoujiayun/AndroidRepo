...
public class CommandSenderTest {
	...
	public void testSendDumpTree() throws JSONException {
		...
		assertThat(echo.getLong("timestamp") >= now, is(true));
		assertThat(echo.getJSONObject("modifiers"), notNullValue());
		assertThat(echo.getJSONObject("modifiers").getLong("timeout"), is((long)CommandSender.DEFAULT_TREE_TIMEOUT));
		assertThat(echo.names().length(), is(4));
	}
}