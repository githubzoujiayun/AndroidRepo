...
public class GeneralActivity extends FragmentActivity {
	...
	private CheckingProgressDialog mDownloadDialog;
	UserInfo mUserInfo;
	Preferences mPrefs;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ACTION_LOGIN.equals(action)) {
			...
			replaceFragment(ClothesRack.class, getIntent().getExtras(),
					R.id.fragment);
	...
	private void onFetchedIds(String mimeType, JSONObject jsonObj)
		...
		if (ClothesInfo.MIMETYPE_IMAGE.equals(mimeType)) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (; j < length; j++) {
					if (ids[i] == localIds[j]) {
						ClothesInfo info = ClothesInfo.getImageInfoBySID(
								resolver, ids[i], userId);
			...
			for (int i = 0; i < localIds.length; i++) {
				...
				if (j == size) {
					ClothesInfo info = ClothesInfo.getImageInfoBySID(resolver,
							localIds[i], userId);
					...
					Uri uri = ContentUris.withAppendedId(ImageInfo.CONTENT_URI,
							info.mId);
			...
			for (int i = 0; i < size; i++) {
				...
				for (; j < length; j++) {
					if (ids[i] == localIds[j]) {
						ClothesInfo info = ClothesInfo.getVideoInfoBySID(
								resolver, ids[i], userId);
						String mediaPath = info.mMediaPath;
						String mediaName = info.mMediaName;
						if (mediaName != null && mediaPath != null
	...
	class PostCallback implements IPostCallback {
			...
			try {
				...
				case PostController.POST_ID_DELETE_IMAGE:
					String userId = Preferences
							.getUsername(GeneralActivity.this);
				...
				case PostController.POST_ID_MODIFY_FILE:
					...
					// String uid =
					// Preferences.getUsername(GeneralActivity.this);
					// ClothesInfo info =
					// ClothesInfo.getImageInfoBySID(resolver,sid, uid);
					// Uri uri =
					// ContentUris.withAppendedId(ImageInfo.CONTENT_URI,
					// info.mId);
					// resolver.delete(uri, null,null);
		...
		public void onPostStart(int post, String info) {
			...
			if (post == PostController.POST_ID_FETCH_FETCH_VIDEO_IDS
				...
				mDownloadDialog = new CheckingProgressDialog();
				if (!mDownloadDialog.isVisible()) {
					// mDownloadDialog.setCancelable(false);
					mDownloadDialog.show(getSupportFragmentManager(), message);
				}
			} else {
				// if (mDownloadDialog !)
			}
