		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG:
				...
				if ("jobcase".equals(tagName)) {
					mRootNode.setResultType(parser.getAttributeValue(null,
							"resultType"));
					mRootNode.setClassName(parser.getAttributeValue(null,
							"className"));
					node = mRootNode;
					...
					String textMatches = parser.getAttributeValue(null,
							"textMatches");
					String textContains = parser.getAttributeValue(null,
							"textContains");
					String textStartsWith = parser.getAttributeValue(null,
							"textStartsWith");
					...
					inputEvent.setTypedChars(parser.getAttributeValue(null,
							"type"));
					node = inputEvent;
					...
					indexView.setIndexs(parser
							.getAttributeValue(null, "indexs"));
					indexView.setRootClass(parser.getAttributeValue(null,
							"rootClass"));
					indexView.setRootIndex(parser.getAttributeValue(null,
							"rootIndex"));
					node = indexView;
				} else if ("wait".equals(tagName)) {
					WaitEvent waitEvent = new WaitEvent();
					waitEvent.setWaitType(parser
							.getAttributeValue(null, "type"));
					waitEvent.setTimeout(parser.getAttributeValue(null,
							"timeout"));
					node = waitEvent;
					...
					pressEvent.setPressKey(parser.getAttributeValue(null,
							"presskey"));
					pressEvent.setKeyCode(parser.getAttributeValue(null,
							"keycode"));
					pressEvent.setMetaState(parser.getAttributeValue(null,
							"metaState"));
					node = pressEvent;
				} else {
				...
				if (node != null) {
					node.setType(Node.ConditionType.toType(parser
							.getAttributeValue(null, "condition")));
					node.setNodeId(parser.getAttributeValue(null,
							"conditionNodeId"));
					node.setId(parser.getAttributeValue(null, "id"));
					node.setResourceId(parser.getAttributeValue(null, "resId"));
					node.setResourceIdMatches(parser.getAttributeValue(null,
							"resIdMatches"));
					node.setClickable(parser.getAttributeValue(null,
							"clickable"));
					node.setEnabled(parser.getAttributeValue(null, "enabled"));
					node.setDescription(parser.getAttributeValue(null,
							"description"));
